...
## Objective
Extract helper functions from main functions into separate subfolders following the one-function-one-file pattern. Transform anonymous functions into named toolkit-style functions.
...
function parseValue(value: string | number): number { // implementation
...
export default function add(a: number): (b: number) => number {
    return function addInner(b: number): number {  // KEEP THIS
        return a + b
    }
}
```
...
items.reduce((a, b) => a + b, 0)        // ERROR: use toolkit add function instead

// AFTER:
import extractId from "./extractId/index.ts"
import hasCountAboveFive from "./hasCountAboveFive/index.ts" 
import add from "../../math/add/index.ts"  // Use existing toolkit function

items.map(extractId)
items.filter(hasCountAboveFive)
items.reduce(add)(0)  // Curried toolkit function
```
...
// NOTE: add function already exists in toolkit - use that instead of creating (a, b) => a + b
```

### Shared Utilities (CHECK USAGE)
```typescript
// BEFORE EXTRACTING - check if function already exists in toolkit
function isString(value: unknown): boolean {  // Check toolkit/validation first
    return typeof value === 'string'
}

function add(a: number, b: number): number {  // Check toolkit/math first  
    return a + b
}

function isEmpty(arr: unknown[]): boolean {   // Check toolkit/array first
    return arr.length === 0
}

// WARNING: Do not extract if equivalent already exists in toolkit!
// Search libraries/toolkit/src/vanilla/ for existing implementations:
...
// If toolkit function exists, import and use that instead of extracting
```
