...
Deno.test("collectScriptElements basic functionality", async (t) => {
	...
	await t.step("should collect scripts from root level", () => {
		const doc = setupDocument(`<div id="root" data-scripts="script1.js,script2.js,script3.js"></div>`)
	...
	await t.step("should ignore non-script attributes", () => {
		const doc = setupDocument(`<div id="test" class="container" data-dependencies="style.css" data-scripts="app.js"></div>`)
	...
	await t.step("should handle component with no scripts or children", () => {
		const doc = setupDocument(`<div id="container" class="container" data-dependencies="style.css"></div>`)
...
Deno.test("collectScriptElements combined scenarios", async (t) => {
	await t.step("should collect scripts from both root and children", () => {
		...
		const result = collectScriptElements(component)
		assertEquals(result, ["main.js", "utils.js", "component1.js", "component2.js", "nested.js"])
	})
	...
	await t.step("should handle complex nested structure with mixed content", () => {
		const doc = setupDocument(`
		...
		const result = collectScriptElements(component)
		assertEquals(result, ["root.js", "header.js", "nav.js", "section.js", "article.js"])
	})
})

Deno.test("collectScriptElements edge cases", async (t) => {
	await t.step("should handle different script value types", () => {
		const doc = setupDocument(`<div id="root" data-scripts="string-script.js,/path/to/script.js,https://cdn.example.com/lib.js"></div>`)
		const element = doc.getElementById("root")!
		const component = createComponentFromElement(element)
		
		const result = collectScriptElements(component)
		assertEquals(result, ["string-script.js", "/path/to/script.js", "https://cdn.example.com/lib.js"])
	})
	...
	await t.step("should handle duplicate script references", () => {
		...
		const result = collectScriptElements(component)
		assertEquals(result, ["common.js", "app.js", "common.js", "module1.js", "module2.js", "app.js"])
	})
...
Deno.test("collectScriptElements realistic scenarios", async (t) => {
	await t.step("should handle typical web application structure", () => {
		...
		const result = collectScriptElements(component)
		assertEquals(result, [
			"polyfills.js", "vendor.js",
			"app.js", "header.js", "nav.js", "dropdown.js",
			"analytics.js", "footer.js"
		])
	})
	...
	await t.step("should handle form with interactive elements", () => {
		...
		assertEquals(result, [
			"form-validation.js",
			"input-mask.js", "autocomplete.js", "select2.js",
			"button-loading.js"
		])
	})
})
...
Deno.test("collectScriptElements property-based tests", () => {
	fc.assert(fc.property(
		fc.array(fc.string({ minLength: 1, maxLength: 20 }).map(s => s + ".js")),
		(scripts) => {
	...
	fc.assert(fc.property(
		fc.array(fc.array(fc.string({ minLength: 1, maxLength: 20 }).map(s => s + ".js"))),
		(childScripts) => {
	...
	fc.assert(fc.property(
		fc.array(fc.string({ minLength: 1, maxLength: 10 }).map(s => s + ".js")),
		fc.array(fc.array(fc.string({ minLength: 1, maxLength: 10 }).map(s => s + ".js"))),
