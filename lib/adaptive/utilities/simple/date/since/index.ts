 ...
 */
const since = (reference: Temporal.PlainDate | Temporal.PlainDateTime | 
	...
	const isValidReference = 
		reference instanceof Temporal.PlainDate ||
	...
	try {
		...
		if (reference instanceof Temporal.PlainTime && current instanceof Temporal.PlainTime) {
			const refSeconds = reference.hour * 3600 + reference.minute * 60 + reference.second
			const curSeconds = current.hour * 3600 + current.minute * 60 + current.second
