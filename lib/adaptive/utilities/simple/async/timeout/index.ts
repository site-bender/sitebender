 ...
 */
const timeout = (milliseconds: number) => <E = Error>(
	error?: E
) => <T>(
	promise: Promise<T>
): Promise<T> => {
	...
	if (milliseconds < 0 || !Number.isFinite(milliseconds)) {
		return Promise.reject(
			new Error(`Invalid timeout: ${milliseconds}ms. Must be a non-negative finite number.`)
		)
	}
	...
	const timeoutPromise = new Promise<never>((_, reject) => {
		setTimeout(() => {
			const timeoutError = error ?? new Error(`Operation timed out after ${milliseconds}ms`)
			reject(timeoutError)
		}, milliseconds)
	})
