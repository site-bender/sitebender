...
function convertJSXToData(jsxElement: { type: unknown; props: unknown }): unknown {
    const { type, props } = jsxElement

    if (typeof type === 'function' && props && typeof props === 'object') {
        const { children, format, element, property, subtypeProperties, ...dataProps } = props as any
