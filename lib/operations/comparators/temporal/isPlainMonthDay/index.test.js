...
test("[isPlainMonthDay] (calculations::comparators::temporal) returns Left(Array(Errors)) when bad operand", async () => {
	expect(
		await isPlainMonthDay(IsPlainMonthDay(Constant("PlainMonthDay")()))(),
	).toMatchObject({
		left: [
			{
				tag: "Error",
