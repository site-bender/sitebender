...
test("[isPlainYearMonth] (calculations::comparators::temporal) returns Left(Array(Errors)) when bad operand", async () => {
	expect(
		await isPlainYearMonth(IsPlainYearMonth(Constant("PlainYearMonth")()))(),
	).toMatchObject({
		left: [
			{
				tag: "Error",
