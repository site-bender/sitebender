...
test("[isAfterDate] (calculations::comparators::date) returns an error when not after date", async () => {
	expect(
		await isAfterDate(
...
test("[isAfterDate] (calculations::comparators::date) returns an error when same date", async () => {
	expect(
		await isAfterDate(
			IsAfterDate("Date")(Constant("Date")("2001-01-01"))(
				Constant("Date")("2001-01-01"),
