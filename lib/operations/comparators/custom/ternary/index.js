...
const ternary = op => async arg => {
	const condition = await composeConditional(op.condition)(arg)
	const ifFalse = await composeComparators(op.ifFalse)(arg)
	const ifTrue = await composeComparators(op.ifTrue)(arg)
