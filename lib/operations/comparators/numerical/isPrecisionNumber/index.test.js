...
test("[isPrecisionNumber] (calculations::comparators::numerical) returns Left(Array(Errors)) when bad operand", async () => {
	expect(
		await isPrecisionNumber(IsPrecisionNumber(Constant("PrecisionNumber")()))(),
	).toMatchObject({
		left: [
			{
				tag: "Error",
