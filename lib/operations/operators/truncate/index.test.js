...
test("[truncate] (calculations::operators) gets truncation of positive number", async () => {
	expect(
		await truncate(Truncate("Number")(2)(Constant("Number")(5.1234)))(),
	).toMatchObject({ right: 5.12 })
})

test("[truncate] (calculations::operators) gets truncation of negative number", async () => {
	expect(
		await truncate(Truncate("Number")(3)(Constant("Number")(-5.1234)))(),
	).toMatchObject({ right: -5.123 })
})

test("[truncate] (calculations::operators) returns Left(Array(Errors)) on bad operand", async () => {
