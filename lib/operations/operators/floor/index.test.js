...
test("[floor] (operations::operators) gets floor of positive number", async () => {
	expect(
		await floor(Floor("Number")(2)(Constant("Number")(5.1234)))(),
	).toMatchObject({ right: 5.12 })
})

test("[floor] (operations::operators) gets floor of negative number", async () => {
	expect(
		await floor(Floor("Number")(3)(Constant("Number")(-5.1234)))(),
	).toMatchObject({ right: -5.124 })
})

test("[floor] (operations::operators) returns Left(Array(Errors)) on bad operand", async () => {
