...
test("[logBaseTwo] (calculations::operators) gets the logBaseTwo of 0", async () => {
	const op = LogBaseTwo("Number")(Constant("Number")(16))

	expect(await logBaseTwo(op)()).toMatchObject({ right: 4 })
})

test("[logBaseTwo] (calculations::operators) returns Left(Array(Errors)) on bad operand", async () => {
