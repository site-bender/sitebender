...
test("[subtract] (operations::operators) returns Right(value)", async () => {
	const op = Subtract("Number")(Constant()(12))(Constant()(7))

	expect(await subtract(op)()).toMatchObject({ right: 5 })
})

test("[subtract] (operations::operators) returns Left(Array(Errors)) on bad minuend", async () => {
...
test("[subtract] (operations::operators) returns Left(Array(Errors)) on bad subtrahend", async () => {
