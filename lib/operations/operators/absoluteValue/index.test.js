...
test("[absoluteValue] (operations::operators) makes negative positive", async () => {
	const op = AbsoluteValue("Number")(FromArgument("Number"))

	expect(await absoluteValue(op)(-21)).toMatchObject({ right: 21 })
})

test("[absoluteValue] (operations::operators) keeps positive positive", async () => {
	const op = AbsoluteValue("Number")(FromArgument("Number"))

	expect(await absoluteValue(op)(42)).toMatchObject({ right: 42 })
})

test("[absoluteValue] (operations::operators) returns Left(Array(Errors)) on bad operand", async () => {
