...
test("[naturalLog] (operations::operators) gets the naturalLog of 0", async () => {
	const op = NaturalLog("Number")(Constant("Number")(Math.E * Math.E))

	expect(await naturalLog(op)()).toMatchObject({ right: 2 })
})

test("[naturalLog] (operations::operators) returns Left(Array(Errors)) on bad operand", async () => {
