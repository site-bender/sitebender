...
test("[root] (operations::operators) returns Right(value)", async () => {
	const op = Root("Number")(Constant()(144))(Constant()(2))

	expect(await root(op)()).toMatchObject({ right: 12 })
})

test("[root] (operations::operators) returns Left(Array(Errors)) on index zero", async () => {
...
test("[root] (operations::operators) returns Left(Array(Errors)) on bad base", async () => {
...
test("[root] (operations::operators) returns Left(Array(Errors)) on bad exponent", async () => {
