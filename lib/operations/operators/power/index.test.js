...
test("[power] (calculations::operators) returns Right(value)", async () => {
	const op = Power("Number")(Constant()(12))(Constant()(2))

	expect(await power(op)()).toMatchObject({ right: 144 })
})

test("[power] (calculations::operators) returns Left(Array(Errors)) on bad base", async () => {
...
test("[power] (calculations::operators) returns Left(Array(Errors)) on bad exponent", async () => {
