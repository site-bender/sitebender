...
test("[min] (calculations::operators) returns Right(value)", async () => {
	const op = Min("Number")([Constant()(1), Constant()(2), Constant()(3)])

	expect(await min(op)()).toMatchObject({ right: 1 })
})

test("[min] (calculations::operators) returns a left error if array empty", async () => {
	const op = Min("Number")([])
...
test("[min] (calculations::operators) returns Left(Array(Errors)) on bad operand", async () => {
