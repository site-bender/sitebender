...
test("[isNotAfterDate] (calculations::comparators::date) returns an error when not after date", async () => {
	expect(
		await isNotAfterDate(
...
test("[isNotAfterDate] (calculations::comparators::date) returns an error when same date", async () => {
	expect(
		await isNotAfterDate(
			IsNotAfterDate("Date")(Constant("Date")("2001-01-01"))(
				Constant("Date")("2001-01-01"),
