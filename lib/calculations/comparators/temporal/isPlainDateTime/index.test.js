...
test("[isPlainDateTime] (calculations::comparators::scalar) returns value when a plain date-time", async () => {
	expect(
		await isPlainDateTime(
...
test("[isPlainDateTime] (calculations::comparators::scalar) returns an error when not a plain date-time", async () => {
	expect(
		await isPlainDateTime(IsPlainDateTime(Constant("String")("Bob")))(),
	).toMatchObject({
		left: [
			{
				tag: "Error",
