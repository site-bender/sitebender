document.addEventListener("DOMContentLoaded", ()=>{
    const formattedFields = document.querySelectorAll(".formatted-field input[data-format]");
    formattedFields.forEach(async (f)=>{
        const field = f;
        const formatName = field.dataset.format;
        const countryCode = field.dataset.countryCode;
        try {
            const { default: format } = await import(`./formatters/${formatName}/index.js`);
            if (formatName === "postalCode" && countryCode) {
                fetch("../../../json/postalCodes/index.json").then((response)=>response.json()).then((POSTAL_CODES)=>{
                    if (POSTAL_CODES[countryCode]) {
                        field.pattern = POSTAL_CODES[countryCode].pattern.replace(/\\/g, '\\\\');
