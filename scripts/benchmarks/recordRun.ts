...
function parseArgs(): Args {
  ...
  for (let i = 0; i < Deno.args.length; i++) {
    ...
    switch (a) {
      ...
      case "--to": out.to = v; i++; break
      case "--label": out.label = v; i++; break
      case "--check": out.check = v; i++; break
      case "--test": out.test = v; i++; break
      case "--lint": out.lint = v; i++; break
      case "--estimate": out.estimate = v; i++; break
...
async function estimateCosts(args?: string) {
  ...
  const { code, out, err } = await sh(`deno run -A scripts/estimate/gptCost.ts ${args}`)
  return { code, out, err }
}
