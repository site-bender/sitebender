...
async function copyDir(srcDir: string, dstDir: string, { skip = new Set<string>() } = {}) {
  for await (const entry of walk(srcDir, { includeDirs: true, includeFiles: true })) {
...
async function updateImports() {
  ...
  for (const base of fileGlobs) {
    try {
      for await (const entry of walk(base, { includeDirs: false, includeFiles: true, exts: ["ts", "tsx"] })) {
...
function rewriteRelativeImports(filePath: string, code: string) {
  ...
  return code.replace(importRE, (_m, prefix, spec) => {
    ...
    const isUnder = (rel: string) => rel && !rel.startsWith("..") && !path.isAbsolute(rel)
...
async function main() {
  ...
  try {
    for await (const entry of walk(SRC_CONSTANTS, { includeDirs: false, includeFiles: true })) {
