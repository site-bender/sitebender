...
Deno.test("processFile", async function testProcessFile(t) {
	...
	await t.step("leaves relative imports unchanged", async function testRelativeImports() {
...
import type { Component } from "libraries/components/types"
`
		await Deno.writeTextFile(filePath, content)
		...
		assertEquals(result.includes("libraries/toolkit/src"), false)
	})

	await t.step("handles mixed import styles", async function testMixedImports() {
		const filePath = path.join(testDir, "mixed.ts")
		const content = `
import { local } from "./local"
import { deep } from "libraries/toolkit/src/utils"
export { something } from "../relative"
export { another } from "libraries/components/src/base"
`
		await Deno.writeTextFile(filePath, content)
		...
		assertEquals(result.includes('"libraries/toolkit'), false)
		assertEquals(result.includes('"libraries/components/src'), false)
	})

	await t.step("preserves single and double quotes", async function testQuotes() {
		const filePathSingle = path.join(testDir, "single.ts")
		const contentSingle = `
import { test } from 'libraries/toolkit/src/test'
`
...
`
		...
		assertEquals(resultDouble.includes("'../") || resultDouble.includes('"/'), false)
	})

	await t.step("handles export statements", async function testExports() {
		const filePath = path.join(testDir, "exports.ts")
		const content = `
export { default } from "libraries/toolkit/src/pipe"
export * from "libraries/components/types"
export { foo, bar } from "libraries/engine/src/utils"
`
		await Deno.writeTextFile(filePath, content)
	...
	await t.step("returns false when no changes needed", async function testNoChanges() {
...
import {
	...
	filter,
	reduce
} from "libraries/toolkit/src/vanilla/array"
`
		await Deno.writeTextFile(filePath, content)
