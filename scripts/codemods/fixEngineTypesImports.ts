...
const TYPES_ROOT = resolve(workspaceRoot, "libraries/engine/types/schema.org")
...
function toSrcDefinePathFromEngineType(typeAbsPath: string): string {
...
async function processFile(
	...
	for (const line of lines) {
		...
		for (const s of specs) {
			...
			if (typeAbs) {
				srcAbs = toSrcDefinePathFromEngineType(typeAbs)
			}

			// 2) Fallback: try to discover the component by walking components/src/define looking for a unique folder name match
			if (!srcAbs) {
