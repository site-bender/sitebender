...
export const TYPE_IMPORT_DEFAULT_REGEX = /import\s+type\s+(\w+)\s+from\s+["'](.+?index\.ts)["'];/g

//++ Regex to match named type imports like: import type { Foo, Bar } from "./types/index.ts"
export const TYPE_IMPORT_NAMED_REGEX = /import\s+type\s+\{\s*([^}]+)\s*\}\s+from\s+["'](.+?index\.ts)["'];/g

//++ Regex to match barrel imports with "as" renaming
export const BARREL_IMPORT_REGEX = /^\s*import\s*\{([^}]*)\}\s*from\s*["'](.+components\/index\.tsx)["'];?\s*$/
