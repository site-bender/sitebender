...
Deno.test(
	...
	async function testShouldProcessDirectory(t) {
		...
		await t.step(
			"allows non-excluded directories",
			function testAllowsNonExcluded() {
		...
		await t.step(
			"handles typical exclusions",
			function testTypicalExclusions() {
