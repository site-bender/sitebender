...
async function main() {
  ...
  const globs = (pedantic ? args.filter((a) => a !== '--pedantic') : args).length
  ...
  const violations: Array<{ file: string; line: number; col: number; rule: string; snippet: string }> = []
      ...
      FORBIDDEN.forEach((rule) => {
        lines.forEach((ln, idx) => {
          if (rule.regex.test(ln)) {
            violations.push({ file, line: idx + 1, col: ln.search(rule.regex) + 1, rule: rule.name, snippet: ln.trim() })
          }
        })
      })
  ...
  let line = 1, col = 1
        ...
        violations.push({ file, line, col, rule: "Object.assign(non-literal-target)", snippet })
      }
    } catch {
