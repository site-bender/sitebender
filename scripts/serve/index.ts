...
export default async function startDevServer(): Promise<void> {
	...
	// Ensure we operate within the docs application so outputs go to applications/mission-control/dist
	const originalCwd = Deno.cwd()
	const repoRoot = dirname(dirname(fromFileUrl(import.meta.url)))
	const docsDir = join(repoRoot, "applications", "mission-control")
	Deno.chdir(docsDir)
