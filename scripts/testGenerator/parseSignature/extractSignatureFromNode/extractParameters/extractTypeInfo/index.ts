...
export default function extractTypeInfo(
	...
	if (ts.isLiteralTypeNode(typeNode)) {
		...
		const value: unknown = ts.isStringLiteral(literal) ? literal.text :
			ts.isNumericLiteral(literal) ? Number(literal.text) :
			literal.kind === ts.SyntaxKind.TrueKeyword ? true :
			literal.kind === ts.SyntaxKind.FalseKeyword ? false :
			undefined
