...
import { join, dirname, relative as relPath } from "jsr:@std/path"
...
function findFilterAttributesBlock(src: string): { start: number; end: number; head: string; param: string } | null {
...
async function processFile(absPath: string): Promise<boolean> {
  ...
  const adjustedImports = importLines.map((l) => l.replace(/from\s+["']([^"']+)["']/, (_s, spec) => `from "${adjustImportPath(String(spec))}"`))

  const typeName = extractTypeName(param)
  const relParent = "../index.ts"
  const importTypeLine = typeName ? `import type { ${typeName} } from "${relParent}"
` : ""

  const funcBody = text.slice(text.indexOf("{", text.indexOf("=>", start)) + 1, end - 1)
  const newContent = [
    ...adjustedImports,
    importTypeLine,
  ...
  beforeLines.splice(importInsertionPoint, 0, 'import filterAttributes from "./filterAttributes/index.ts"')
  ...
  console.log(`Refactored ${relPath(ROOT, absPath)} -> ${relPath(ROOT, childFile)}`)
  return true
}
