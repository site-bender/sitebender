...
const globWalk = async function* (root: string): AsyncGenerator<string, any, any> {
    ...
    for await (const file of globWalk(root)) {
      ...
      lines.forEach((line, idx) => {
        ...
        while ((m = importRe.exec(line))) {
          ...
          if (spec.startsWith("@adaptiveSrc/") || spec.startsWith("@adaptiveTypes/") || spec.startsWith("@toolkit/")) {
            continue
          }
          ...
          if (!isInside(file, "libraries/adaptive")) {
            ...
            if (spec.includes(ADAPTIVE_TYPES)) {
              violations.push({
                ...
                line: idx + 1,
                spec,
                hint: "Use @adaptiveTypes/… instead of libraries/adaptive/types/…",
              })
            }
          }
