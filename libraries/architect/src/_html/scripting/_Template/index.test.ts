...
import _Template from "./index.ts"

Deno.test("_Template component", async function templateTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Template({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Template({ children: [paragraph] })
