...
import _Colgroup from "./index.ts"

Deno.test("_Colgroup component", async function colgroupTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Colgroup({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Colgroup({ children: [paragraph] })
