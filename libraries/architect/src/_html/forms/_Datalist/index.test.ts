...
import _Datalist from "./index.ts"

Deno.test("_Datalist component", async function datalistTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Datalist({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Datalist({ children: [paragraph] })
