...
import _Textarea from "./index.ts"

Deno.test("_Textarea component", async function textareaTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Textarea({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Textarea({ children: [paragraph] })
