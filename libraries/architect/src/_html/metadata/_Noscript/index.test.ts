...
import _Noscript from "./index.ts"

Deno.test("_Noscript component", async function noscriptTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Noscript({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Noscript({ children: [paragraph] })
