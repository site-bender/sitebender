...
import _Picture from "./index.ts"

Deno.test("_Picture component", async function pictureTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Picture({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Picture({ children: [paragraph] })
