...
import _Details from "./index.ts"

Deno.test("_Details component", async function detailsTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Details({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Details({ children: [paragraph] })
