...
import _Blockquote from "./index.ts"

Deno.test("_Blockquote component", async function blockquoteTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Blockquote({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Blockquote({ children: [paragraph] })
