...
import _Address from "./index.ts"

Deno.test("_Address component", async function addressTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Address({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Address({ children: [paragraph] })
