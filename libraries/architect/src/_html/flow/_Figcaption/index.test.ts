...
import _Figcaption from "./index.ts"

Deno.test("_Figcaption component", async function figcaptionTests(t) {
	await t.step(
		"returns a VirtualNode",
		function returnsVirtualNode() {
			const component = _Figcaption({})
	...
	await t.step(
		...
		function handlesChildren() {
			...
			const component = _Figcaption({ children: [paragraph] })
