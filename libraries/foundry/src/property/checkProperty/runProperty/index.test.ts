...
import type { Property, PropertyFailure, PropertySuccess } from "../../../types/index.ts"
...
Deno.test("runProperty - short-circuits on existing failure", () => {
	const existingError = err({ type: "failure" as const, counterexample: [1], seed: 999, shrinks: 0 })
...
Deno.test("runProperty - propagates generator failures", () => {
	const property: Property<[number]> = {
		...
		arbitraries: [{ generator: (_s) => err({ type: "GenerationFailed", reason: "Test failure" }) }] as const,
		predicate: () => true
	}
