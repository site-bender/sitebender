...
export default function generateContractDoc(contract: Contract): string {
	const libraryName = contract.library.charAt(0).toUpperCase() + contract.library.slice(1)
	...
	if (contract.responsibilities) {
		...
		if (contract.responsibilities.owns) {
			sections.push(subsection(`### ${libraryName} Owns`, contract.responsibilities.owns))
		}
		if (contract.responsibilities.provides) {
			sections.push(subsection(`### ${libraryName} Provides`, contract.responsibilities.provides))
		}
		if (contract.responsibilities.consumes) {
			sections.push(subsection(`### ${libraryName} Consumes`, contract.responsibilities.consumes))
		}
		if (contract.responsibilities.forbidden) {
			sections.push(subsection(`### ${libraryName} Must Never`, contract.responsibilities.forbidden, "- ❌ "))
		}
	}
	...
	if (contract.implementation) {
		...
		if (contract.implementation.forbidden) {
			...
			sections.push(subsection(title, contract.implementation.forbidden, "- ❌ "))
		}
	}
	
	// Output Requirements (Parser style)
	if (contract.output?.requirements) {
		sections.push(section("## Output Requirements", [`Every output from ${libraryName} must:`]))
		sections.push(subsection("", contract.output.requirements))
	}
	
	// Input Requirements (Envoy style)
	if (contract.input?.requirements) {
		sections.push(section("## Input Requirements", [`Every input to ${libraryName} must:`]))
	...
	if (validation) {
		...
		if (validation["compile-time"]) {
			sections.push(subsection("### Compile-Time Validation", validation["compile-time"]))
		}
		if (validation.runtime) {
			sections.push(subsection("### Runtime Validation", validation.runtime))
		}
		if (validation["test-time"]) {
			sections.push(subsection("### Test-Time Validation", validation["test-time"]))
		}
	}
	...
	if (contract.consumers) {
		...
		if (contract.consumers.allowed) {
			sections.push(subsection(`### Allowed to consume ${libraryName} output:`, contract.consumers.allowed, "- ✅ "))
		}
		if (contract.consumers.forbidden) {
			sections.push(subsection(`### Forbidden from consuming ${libraryName} output:`, contract.consumers.forbidden, "- ❌ "))
		}
	}
	...
	if (contract.dependencies) {
		...
		if (contract.dependencies.allowed) {
			sections.push(subsection("### Allowed dependencies:", contract.dependencies.allowed, "- ✅ "))
		}
		if (contract.dependencies.forbidden) {
			sections.push(subsection("### Forbidden dependencies:", contract.dependencies.forbidden, "- ❌ "))
		}
	}
