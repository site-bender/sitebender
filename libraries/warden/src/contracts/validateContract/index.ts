  ...
  return async function () {
    try {
      ...
      const errors = filter((x: string | false) => Boolean(x))([
        ...
        !(contract.api && contract.api.exports) && "Contract missing required 'api.exports' field"
      ])

      // Build warnings array using proper functional composition
      const warnings = filter((x: string | false) => Boolean(x))([
        !contract.privacy && "Contract missing 'privacy' field - privacy rules will not be enforced",
        !contract.implementation && "Contract missing 'implementation' field - implementation constraints will not be enforced"
      ])
