...
import type { WardenConfig, EnforcementPhase, Violation } from "../../types/index.ts"
...
export default function enforce(config: WardenConfig) {
  return async function enforcePhase(phase: EnforcementPhase = "warn") {
    ...
    const createTargetValidator = (target: string) => async () => {
      ...
      const [structureValid, privacyValid, importValid, contractValid] = await Promise.all([
      ...
      if (!contractValid.success) {
        targetViolations.push({
          type: "contract",
          severity: "medium",
          message: `Contract validation failed for ${target}: ${contractValid.errors.join(", ")}`,
          file: target
        })
      }
