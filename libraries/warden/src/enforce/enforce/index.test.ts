...
Deno.test("enforce function works with basic configuration", async () => {
  ...
  console.assert(Array.isArray(result.violations), "Result should have violations array")
  console.assert(typeof result.executionTime === "number", "Result should have executionTime")
...
Deno.test("enforce function handles empty targets", async () => {
  ...
  console.assert(result.violations.length === 0, "Empty targets should have no violations")
})
