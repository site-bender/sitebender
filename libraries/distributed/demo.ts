...
async function demoIPFS() {
	...
	try {
		...
		console.log(`${colors.yellow}Note: IPFS gateways may be slow or unavailable${colors.reset}`)
...
async function demoStorage() {
	...
	const storage = {
		save: async (key: string, data: unknown) => {
			console.log(`${colors.cyan}[Storage]${colors.reset} Saving key: ${key}`)
			info("Data", JSON.stringify(data, null, 2))
			success("Data saved to IndexedDB")
		},
		load: async (key: string) => {
			console.log(`${colors.cyan}[Storage]${colors.reset} Loading key: ${key}`)
			success("Data loaded from IndexedDB")
			return null
		},
		getAllKeys: async () => {
			console.log(`${colors.cyan}[Storage]${colors.reset} Getting all keys`)
			return ["crdt:1", "crdt:2", "crdt:3"]
		},
		delete: async (key: string) => {
			console.log(`${colors.cyan}[Storage]${colors.reset} Deleting key: ${key}`)
