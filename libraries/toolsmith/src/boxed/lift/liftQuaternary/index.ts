...
export default function liftQuaternary<A, B, C, D, E, F>(
	fn: (
		a: A,
	) => ((b: B) => ((c: C) => ((d: D) => E | null) | null) | null) | null,
) {
				...
				return function liftedFourth(
					...
					if (
						...
						const aVal = isValidation(ma)
							...
							? (isOk(ma)
								? success(ma.value)
								: { _tag: "Failure" as const, errors: [ma.error] as [F] })
							: success(ma as A)
						...
						const bVal = isValidation(mb)
							...
							? (isOk(mb)
								? success(mb.value)
								: { _tag: "Failure" as const, errors: [mb.error] as [F] })
							: success(mb as B)
						...
						const cVal = isValidation(mc)
							...
							? (isOk(mc)
								? success(mc.value)
								: { _tag: "Failure" as const, errors: [mc.error] as [F] })
							: success(mc as C)
						...
						const dVal = isValidation(md)
							...
							? (isOk(md)
								? success(md.value)
								: { _tag: "Failure" as const, errors: [md.error] as [F] })
							: success(md as D)

						const result = validationMap4(
							fn as (a: A) => (b: B) => (c: C) => (d: D) => E,
						)(aVal as Validation<F, A>)(bVal as Validation<F, B>)(
							cVal as Validation<F, C>,
						)(dVal as Validation<F, D>)

						// Check if result is Success(null) and convert to Failure
						if (isValid(result) && result.value === null) {
							return failure([
								"Null return from function" as unknown as F,
							]) as Validation<F, E>
						}
					...
					const result = resultMap4(
						fn as (a: A) => (b: B) => (c: C) => (d: D) => E,
					)(aResult as Result<F, A>)(bResult as Result<F, B>)(
						cResult as Result<F, C>,
					)(dResult as Result<F, D>)
