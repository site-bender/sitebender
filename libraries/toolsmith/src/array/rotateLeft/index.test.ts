...
import rotateLeft from "./index.ts"
import ok from "../../monads/result/ok/index.ts"
import error from "../../monads/result/error/index.ts"
import success from "../../monads/validation/success/index.ts"
import failure from "../../monads/validation/failure/index.ts"

describe("rotateLeft", function testRotateLeft() {
	describe("Plain array path", function testPlainArrayPath() {
		it("rotates elements left by 1 position", function testBasicRotateLeft() {
			const input = [1, 2, 3, 4, 5]
			const result = rotateLeft(1)(input)
			assertEquals(result, [2, 3, 4, 5, 1])
		})

		it("rotates elements left by 2 positions", function testRotateLeftBy2() {
			const input = [1, 2, 3, 4, 5]
			const result = rotateLeft(2)(input)
		...
		it("handles large arrays efficiently", function testLargeArrays() {
			...
			const result = rotateLeft(100)(input)
			...
			fc.assert(
				fc.property(
					...
					function testProperty(
						...
						const result = rotateLeft(positions)([])
						assertEquals(result, [])
					},
				),
			)
			...
			fc.assert(
				fc.property(
					...
					function testProperty(
						...
						const result = rotateLeft(positions)(arr)
						//++ [EXCEPTION] Using array.length for assertion
			...
			fc.assert(
				fc.property(
					...
					function testProperty(
						...
						const result1 = rotateLeft(positions2)(rotateLeft(positions1)(arr))
						const result2 = rotateLeft(positions1 + positions2)(arr)
