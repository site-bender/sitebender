...
export default function formatIsbn10(): (isbn10: Isbn10) => string {
	return function formatToDisplay(isbn10: Isbn10): string {
		//++ [EXCEPTION] Array indexing [] and .slice() permitted in Toolsmith for performance - provides ISBN-10 formatting wrapper
		return `${isbn10[0]}-${isbn10.slice(1, 4)}-${isbn10.slice(4, 9)}-${
			isbn10[9]
		}`
	}
}
