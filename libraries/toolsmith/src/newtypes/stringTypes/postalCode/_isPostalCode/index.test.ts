...
import _isPostalCode from "./index.ts"
...
Deno.test("_isPostalCode returns true for valid 5-digit ZIP codes", function returnsTrueForValid5Digit() {
	...
	assertEquals(_isPostalCode("90210"), true) // Beverly Hills
	assertEquals(_isPostalCode("10001"), true) // New York
	assertEquals(_isPostalCode("60601"), true) // Chicago
	assertEquals(_isPostalCode("77001"), true) // Houston
	assertEquals(_isPostalCode("33101"), true) // Miami
})
...
Deno.test("_isPostalCode returns true for valid 9-digit ZIP codes", function returnsTrueForValid9Digit() {
	...
	assertEquals(_isPostalCode("902101234"), true) // Beverly Hills +4
	assertEquals(_isPostalCode("100011234"), true) // New York +4
	assertEquals(_isPostalCode("606011234"), true) // Chicago +4
})

Deno.test("_isPostalCode returns true for valid ZIP+4 format", function returnsTrueForValidZipPlus4() {
	// Known valid ZIP+4 format
	assertEquals(_isPostalCode("90210-1234"), true) // Beverly Hills
	assertEquals(_isPostalCode("10001-5678"), true) // New York
	assertEquals(_isPostalCode("60601-9012"), true) // Chicago
})

Deno.test("_isPostalCode returns false for wrong length strings", function returnsFalseForWrongLength() {
	assertEquals(_isPostalCode(""), false) // Empty
	assertEquals(_isPostalCode("1234"), false) // Too short
	assertEquals(_isPostalCode("123456"), false) // Invalid length
	assertEquals(_isPostalCode("1234567"), false) // Invalid length
	assertEquals(_isPostalCode("12345678"), false) // Invalid length
	assertEquals(_isPostalCode("1234567890"), false) // Too long
	assertEquals(_isPostalCode("12345678901"), false) // Much too long
})

Deno.test("_isPostalCode returns false for strings with invalid characters", function returnsFalseForInvalidChars() {
	assertEquals(_isPostalCode("9021A"), false) // Letter in 5-digit
	assertEquals(_isPostalCode("90210-123A"), false) // Letter in ZIP+4
	assertEquals(_isPostalCode("90210 1234"), false) // Space instead of hyphen
	assertEquals(_isPostalCode("90210_1234"), false) // Underscore instead of hyphen
	assertEquals(_isPostalCode("90210.1234"), false) // Dot instead of hyphen
})

Deno.test("_isPostalCode returns false for invalid ZIP+4 format", function returnsFalseForInvalidZipPlus4() {
	assertEquals(_isPostalCode("902101234-"), false) // Hyphen at end
	assertEquals(_isPostalCode("-902101234"), false) // Hyphen at start
	assertEquals(_isPostalCode("90210-123"), false) // Too few digits after hyphen
	assertEquals(_isPostalCode("90210-12345"), false) // Too many digits after hyphen
	assertEquals(_isPostalCode("9021-01234"), false) // Too few digits before hyphen
	assertEquals(_isPostalCode("902101-234"), false) // Too many digits before hyphen
})
...
Deno.test("_isPostalCode returns false for non-string inputs", function returnsFalseForNonStrings() {
	...
	assertEquals(_isPostalCode(90210 as any), false)
	assertEquals(_isPostalCode(null as any), false)
	assertEquals(_isPostalCode(undefined as any), false)
	assertEquals(_isPostalCode({} as any), false)
	assertEquals(_isPostalCode([] as any), false)
})
...
Deno.test("_isPostalCode - property: valid postal codes pass", function validPostalCodesPass() {
	...
	validPostalCodes.forEach(function testValidPostalCode(code) {
		assertEquals(_isPostalCode(code), true, `Expected ${code} to be valid`)
	})
})
	...
	fc.assert(
		fc.property(
			...
			function propertyInvalidLength(value) {
				assertEquals(_isPostalCode(value), false)
			},
		),
	)
})
	...
	fc.assert(
		fc.property(
			...
			function propertyIdempotent(value) {
				const first = _isPostalCode(value)
				const second = _isPostalCode(value)
	...
	fc.assert(
		fc.property(
			...
			function propertyNonStrings(value) {
				assertEquals(_isPostalCode(value as any), false)
			},
		),
	)
})
