...
//++ Checks if a value is between min (inclusive) and max (exclusive): min <= value < max
export default function betweenMinInclusive(min: number) {
	return function betweenMinInclusiveWithMin(max: number) {
		//++ [OVERLOAD] Number validator: takes a number, returns a boolean
		function betweenMinInclusiveWithMinMax(value: number): boolean

		//++ [OVERLOAD] Result validator: takes and returns a Result monad (fail fast)
		function betweenMinInclusiveWithMinMax(
		...
		return betweenMinInclusiveWithMinMax
	}
}
