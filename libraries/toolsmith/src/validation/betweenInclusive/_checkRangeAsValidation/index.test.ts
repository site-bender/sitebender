...
Deno.test("_checkRangeAsValidation returns success(true) when value is between min and max inclusive", function () {
...
Deno.test("_checkRangeAsValidation returns success(true) when value equals min", function () {
...
Deno.test("_checkRangeAsValidation returns failure when value is below min", function () {
	...
	assertEquals(result.errors[0].received, 0)
})
...
Deno.test("_checkRangeAsValidation returns failure when value is above max", function () {
	...
	assert(isFailure(result))
	assertEquals(result.errors[0].messages[0], "Value must be between 1 and 10 (inclusive)")
})
