...
//++ Checks if a value is between min and max (inclusive: min <= value <= max)
export default function betweenInclusive(min: number) {
	return function betweenInclusiveWithMin(max: number) {
		//++ [OVERLOAD] Number validator: takes a number, returns a boolean
		function betweenInclusiveWithMinMax(value: number): boolean

		//++ [OVERLOAD] Result validator: takes and returns a Result monad (fail fast)
		function betweenInclusiveWithMinMax(
		...
		return betweenInclusiveWithMinMax
	}
}
