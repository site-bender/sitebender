...
Deno.test("_checkRangeAsResult returns ok(true) when value is strictly between min and max", function () {
...
Deno.test("_checkRangeAsResult returns error when value is below min", function () {
	...
	assert(isError(result))
	assertEquals(result.error.messages[0], "Value must be between 1 and 10 (exclusive)")
})
