...
//++ Private helper that checks if a value is between min (exclusive) and max (inclusive), returning a Result
export default function _checkRangeAsResult<E>(min: number) {
	return function checkRangeAsResultWithMin(max: number) {
		return function checkRangeAsResultWithMinAndMax(
			...
			const validationError = _createRangeError<E>(min)(max)(value)("maxInclusive")
