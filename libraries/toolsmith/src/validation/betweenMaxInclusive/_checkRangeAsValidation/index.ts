...
//++ Private helper that checks if a value is between min (exclusive) and max (inclusive), returning a Validation
export default function _checkRangeAsValidation(min: number) {
	return function checkRangeAsValidationWithMin(max: number) {
		return function checkRangeAsValidationWithMinAndMax(
			...
			const validationError = _createRangeError(min)(max)(value)("maxInclusive")
