...
Deno.test("fold", async (t) => {
	await t.step("applies onOk handler for Ok result", () => {
		const result = fold((e: string) => `Error: ${e}`)((x: number) =>
			`Success: ${x}`
		)(ok(42))
	...
	await t.step("can transform to different type", () => {
		const toNumber = fold((_e: string) => 0)((x: number) => x)
	...
	await t.step("works with complex types", () => {
		...
		const handleResult = fold((e: ApiError) => `Error ${e.code}: ${e.message}`)(
			(u: User) => `User: ${u.name}`,
		)
	...
	await t.step("returns same type from both branches", () => {
		const toBoolean = fold((_: string) => false)((x: number) => x > 0)
