...
async function validateNoClasses(
	path: string,
	...
	if (classMatches) {
		result.violations.push(
			`HERESY! Found ${classMatches.length} class(es) in ${path}`,
		)
		result.stats.classesFound++
	}
}

async function validateOneFunctionPerFile(
	path: string,
	result: ValidationResult,
): Promise<void> {
	const content = await Deno.readTextFile(path)
	...
	const defaultFunctionMatches =
		content.match(/^export\s+default\s+(async\s+)?function\s+/gm) || []
	const namedFunctionMatches =
		content.match(/^export\s+(?!default)(async\s+)?function\s+/gm) || []
	const arrowMatches = content.match(
		/^export\s+(const|let)\s+\w+\s*=\s*(\([^)]*\)|[^=])\s*=>/gm,
	) || []
	...
	const totalExports = defaultFunctionMatches.length +
		namedFunctionMatches.length + arrowMatches.length

	if (totalExports > 1) {
		result.violations.push(
			`Multiple functions in ${path}: found ${totalExports} exports`,
		)
	}
...
function validateFolderStructure(path: string, result: ValidationResult): void {
	...
	if (
		!/^[a-z][a-zA-Z0-9]*$/.test(folderName) &&
		folderName !== "types" &&
		folderName !== "constants" &&
		folderName !== "testGenerator"
	) {
		result.violations.push(
			`Folder name not camelCase: ${folderName} in ${path}`,
		)
		return
	}
...
if (import.meta.main) {
	...
	console.log(
		`   Files with correct structure: ${result.stats.correctStructure}`,
	)
