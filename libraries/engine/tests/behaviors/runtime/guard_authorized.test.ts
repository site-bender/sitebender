...
import { guardAuthorized, guardRoute } from "../../../src/runtime/guard/index.ts"

Deno.test("guardAuthorized allows when policy passes", async () => {
  const ctx = createComposeContext({ env: "server", localValues: { user: { id: 1, roles: ["admin"] } } })
...
Deno.test("guardRoute returns redirect when policy fails and redirect provided", async () => {
  ...
  const res = await guardRoute(ctx, "IsAuthenticated", undefined, { redirect: "/login" })
