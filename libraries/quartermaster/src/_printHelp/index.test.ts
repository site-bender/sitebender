...
Deno.test("_printHelp", async function testPrintHelp(t) {
	...
	await t.step(
		"defers execution until IO is run",
		function testDefersExecution() {
			...
			assertEquals(
				callCount,
				0,
				"console.log should not be called when creating IO",
			)
			...
			assertEquals(
				callCount,
				1,
				"console.log should be called exactly once when running IO",
			)
	...
	await t.step(
		"prints the correct help message",
		function testPrintsCorrectMessage() {
			const originalLog = console.log
			let capturedOutput = ""
			...
			assertEquals(
				capturedOutput,
				HELP_MESSAGE,
				"Should print the exact HELP_MESSAGE constant",
			)
	...
	await t.step(
		"multiple calls return equivalent IOs (referential transparency)",
		function testReferentialTransparency() {
			const originalLog = console.log
			let output1 = ""
			let output2 = ""
			...
			assertEquals(
				output1,
				HELP_MESSAGE,
				"Output should match HELP_MESSAGE constant",
			)
