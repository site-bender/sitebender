...
export default function _validateBlueprint(blueprint: Blueprint) {
	return function validateBlueprintWithConfig(): Result<
		ReadonlyArray<ProcessError>,
		Blueprint
	> {
		const basicErrors: ReadonlyArray<ProcessError | null> = [
			// Check required string fields
			!blueprint.id || typeof blueprint.id !== "string" ||
				blueprint.id.trim() === ""
				? {
				...
				: null,
			!blueprint.name || typeof blueprint.name !== "string" ||
				blueprint.name.trim() === ""
				? {
					_tag: "ValidationError" as const,
				...
				: null,
			!blueprint.description || typeof blueprint.description !== "string" ||
			...
			!blueprint.outputs
				...
				: null,
			blueprint.outputs &&
				(!blueprint.outputs.appPath ||
					typeof blueprint.outputs.appPath !== "string" ||
					blueprint.outputs.appPath.trim() === "")
				? {
					_tag: "ValidationError" as const,
			...
			blueprint.outputs && blueprint.outputs.appPath &&
				(blueprint.outputs.appPath.includes("..") ||
					blueprint.outputs.appPath.startsWith("/"))
				? {
		...
		const filteredBasicErrors = basicErrors.filter((
			error,
		): error is ProcessError => error !== null)

		// Validate file specs if present
		const fileValidationResults: ReadonlyArray<
		...
		const fileErrors: ReadonlyArray<ProcessError> = fileValidationResults
			.filter((
				result,
			): result is Result<ReadonlyArray<ProcessError>, FileSpec> & {
				_tag: "Error"
			} => result._tag === "Error")
			.map((result) => result.error)
			.reduce(
				(accumulator, errors) => [...accumulator, ...errors],
				[] as ReadonlyArray<ProcessError>,
			)
