...
Parser will provide Either-compatible constructors while maintaining backward compatibility:
...
Parser will collect the following metadata during initial traversal:
...
`libraries/toolkit/src/ast/` - New directory for shared AST utilities
...
1. **Performance**: Parser must remain fast for large files (< 1s for 500 functions)
