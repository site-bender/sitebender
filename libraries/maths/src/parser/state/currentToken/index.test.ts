...
describe("currentToken", () => {
	it("returns the token at current position", () => {
		const tokens: Token[] = [
	...
	it("returns first token when position is 0", () => {
		const tokens: Token[] = [
	...
	it("returns EOF when position is beyond token array", () => {
		const tokens: Token[] = [
			{ type: "NUMBER", value: "1", position: 0 },
		]
	...
	it("returns EOF with position 0 for empty token array", () => {
		const state: ParserState = {
			tokens: [],
			position: 0,
		}
	...
	it("does not modify the state", async () => {
		const tokens: Token[] = [
		...
		const { default: execState } = await import("../../../../../toolkit/src/monads/state/execState/index.ts")
