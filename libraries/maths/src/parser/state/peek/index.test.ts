...
describe("peek", () => {
	it("looks ahead one token by default", () => {
		const tokens: Token[] = [
	...
	it("looks ahead multiple tokens with offset", () => {
		const tokens: Token[] = [
		...
		// Peek 1 ahead
		assertEquals(evalState(peek(1))(state), { type: "PLUS", value: "+", position: 2 })
		
		// Peek 2 ahead
		assertEquals(evalState(peek(2))(state), { type: "NUMBER", value: "3", position: 3 })
		
		// Peek 3 ahead
		assertEquals(evalState(peek(3))(state), { type: "RIGHT_PAREN", value: ")", position: 4 })
	})
	
	it("returns EOF when peeking past end", () => {
		const tokens: Token[] = [
	...
	it("does not modify state position", () => {
		const tokens: Token[] = [
	...
	it("handles peeking from EOF position", () => {
		const tokens: Token[] = [
			{ type: "NUMBER", value: "1", position: 0 },
	...
	it("can peek at current position with offset 0", () => {
		const tokens: Token[] = [
