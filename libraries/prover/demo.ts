...
async function runDemo() {
	...
	const testTargets = [
		{
			path: `${projectRoot}/libraries/toolkit/src/simple/string/trimStart/index.ts`,
		...
		{
			path: `${projectRoot}/libraries/toolkit/src/simple/combinator/compose/index.ts`,
	...
	for (const target of testTargets) {
		...
		try {
			...
			console.log(`  • Test cases generated: ${BOLD}${suite.testCases.length}${RESET}`)
			console.log(`  • Coverage achieved: ${BOLD}${suite.coverage.percentage.toFixed(1)}%${RESET}`)
			...
			try {
				...
				if (code === 0) {
					...
					console.log(`${YELLOW}⚠️  Tests not yet runnable (may need implementation)${RESET}`)
					if (errorOutput) {
						console.log(`${RED}Error: ${errorOutput.slice(0, 200)}...${RESET}`)
					}
				}
			} catch (e) {
	...
	console.log(`  ${BLUE}import generateTests from "@sitebender/prover"${RESET}`)
