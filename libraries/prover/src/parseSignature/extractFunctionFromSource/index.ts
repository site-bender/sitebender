...
export default function extractFunctionFromSource(
	...
	const processNode = (node: ts.Node): ts.FunctionDeclaration | ts.FunctionExpression | ts.ArrowFunction | null => {
		if (ts.isExportAssignment(node) && node.isExportEquals === false) {
			...
			if (declaration && ts.isVariableDeclaration(declaration)) {
				...
				if (initializer && (ts.isFunctionExpression(initializer) || ts.isArrowFunction(initializer))) {
					...
					const isExported = findExportForName(sourceFile, functionName)
					if (isExported) {
						return initializer
					}
				}
	...
	return children.reduce<ts.FunctionDeclaration | ts.FunctionExpression | ts.ArrowFunction | null>(
