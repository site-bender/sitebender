...
export default function findMinimalBranchSet(tests: Array<TestCase>): Array<TestCase> {
	...
	const findMinimalSetRecursive = (
		...
		if (coveredSoFar.size >= allBranches.size || remainingTests.length === 0) {
		...
		const best = testWithCoverage.reduce((acc, curr) => 
			curr.newCoverageCount > acc.newCoverageCount ? curr : acc,
			{ test: null as TestCase | null, newCoverageCount: 0, newBranches: [] as string[] }
		)
		...
		if (best.test && best.newCoverageCount > 0) {
			...
			return findMinimalSetRecursive(newRemaining, newCovered, newSelected)
		}
