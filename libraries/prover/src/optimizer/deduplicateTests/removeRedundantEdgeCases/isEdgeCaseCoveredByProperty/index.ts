...
export default function isEdgeCaseCoveredByProperty(
	...
	return propertyTest.properties.some(prop => {
		const generator = prop.generator
		
		// Empty array covered by fc.array
		if (Array.isArray(input) && input.length === 0 && generator.includes("fc.array")) {
			return true
		}
		
		// Null/undefined covered by fc.option or fc.constantFrom
		if ((input === null || input === undefined) && 
			(generator.includes("fc.option") || generator.includes("fc.constantFrom"))) {
			return true
		}
