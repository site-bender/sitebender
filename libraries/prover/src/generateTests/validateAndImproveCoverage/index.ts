import type { TestCase, GeneratorConfig, CoverageResult } from "../../types/index.ts"
...
export default async function validateAndImproveCoverage(
	...
	try {
		...
		if (coverage.percentage < config.targetCoverage) {
			console.log(`⚠️  Coverage ${coverage.percentage}% is below target ${config.targetCoverage}%`)
		}
