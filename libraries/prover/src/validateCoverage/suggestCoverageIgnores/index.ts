	...
	try {
		...
		for (const lineNum of uncoveredLines) {
			...
			if (trimmed === "}" || trimmed === "})" || trimmed === "})") {
				suggestions.push(`Line ${lineNum}: Closing brace (safe to ignore)`)
			} else if (trimmed.startsWith("export default")) {
				suggestions.push(`Line ${lineNum}: Export statement (safe to ignore)`)
			} else if (trimmed.startsWith("import ")) {
				suggestions.push(`Line ${lineNum}: Import statement (safe to ignore)`)
			} else if (trimmed.startsWith("//")) {
				suggestions.push(`Line ${lineNum}: Comment (safe to ignore)`)
			} else if (trimmed.includes("/* c8 ignore") || trimmed.includes("deno-coverage-ignore")) {
				suggestions.push(`Line ${lineNum}: Already has coverage ignore`)
			} else if (trimmed.includes("throw") && trimmed.includes("Error")) {
				suggestions.push(`Line ${lineNum}: Error case - may need specific test or ignore`)
			} else if (trimmed === "default:") {
				suggestions.push(`Line ${lineNum}: Default case - may be unreachable`)
			} else {
				suggestions.push(`Line ${lineNum}: Review needed - "${trimmed.substring(0, 50)}..."`)
			}
		}
	} catch (error) {
