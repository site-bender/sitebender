...
**Current Phase**: Planning complete, awaiting Toolsmith boxed function implementation

**Prerequisite**: Toolsmith boxed functions (see `docs/boxed-required.md`)

The lexer implementation requires specific lifted monadic functions from Toolsmith's `boxed` module. These functions wrap vanilla utilities to work with Result/Validation monads, enabling pure FP error handling without exceptions.

**Required before starting**:

- String operations: `charCodeAt`, `charAt`, `slice`, `stringLength`
- Object lookup: `lookup`, `has`
- Composition: `pipe`, `compose`, `identity`

See [boxed-required.md](./boxed-required.md) for complete list with priorities.
...
export const parseNextToken =
  (input: string) =>
  (position: number): TokenParseResult => {
    const char = input[position];
