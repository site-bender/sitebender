...
import type { InjectorNode, OperatorNode, ComparatorNode, ActionNode } from "../../../types/ir/index.ts"

// Helpers
const toNumber = (v: unknown): number => typeof v === 'number' ? v : (typeof v === 'string' && v.trim() !== '') ? Number(v) : 0
const toString = (v: unknown): string => typeof v === 'string' ? v : (v === null || v === undefined) ? '' : String(v)

export function registerDefaultExecutors(_ctx?: ComposeContext) {
  // Injectors
  registerInjector("From.Constant", (node: InjectorNode) => (node.args as { value?: unknown }).value)
  registerInjector("From.Element", (_node: InjectorNode) => {
    const selector = String((_node.args as { selector?: unknown }).selector ?? '')
  ...
  registerComparator("Is.NoShorterThan", async (node: ComparatorNode, evalArg) => {
  ...
  registerComparator("Is.EmailAddress", async (node: ComparatorNode, evalArg) => {
  ...
  registerComparator("Is.Not", async (node: ComparatorNode, evalArg) => {
    const v = await (node.args[0] ? evalArg(node.args[0]) : Promise.resolve(false))
    return !v
  })
  ...
  registerAction("Act.SetQueryString", async (node: ActionNode, evalArg) => {
    if (typeof globalThis === 'undefined' || !("location" in globalThis) || !("history" in globalThis)) return
  ...
  // Events
  const bind = (name: string) => registerEvent(`On.${name}`, (el, _node, dispatch) => {
