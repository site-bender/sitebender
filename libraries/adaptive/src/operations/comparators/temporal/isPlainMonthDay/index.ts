...
const isPlainMonthDay =
	...
	async (
		...
		try {
			...
			return MD.test(s) ? { right: true } : { left: [Error(op.tag)("IsPlainMonthDay")(`${JSON.stringify(operand.right)} is not a plain month-day.`)] }
		} catch (e) {
			return { left: [Error(op.tag)("IsPlainMonthDay")(`${JSON.stringify(operand.right)} is not a plain month-day: ${e}`)] }
		}
	}
