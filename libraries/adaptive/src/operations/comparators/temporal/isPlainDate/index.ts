...
const isPlainDate =
	...
	async (
		...
		try {
			...
			return YMD.test(s) ? { right: true } : { left: [Error(op.tag)("IsPlainDate")(`${JSON.stringify(operand.right)} is not a plain date.`)] }
		} catch (e) {
			return {
				left: [
					Error(op.tag)("IsPlainDate")(`${JSON.stringify(operand.right)} is not a plain date: ${e}.`),
				],
			}
		}
	}
