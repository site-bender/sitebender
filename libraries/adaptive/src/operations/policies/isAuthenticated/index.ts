import type { AdaptiveError, Either, LocalValues, OperationFunction } from "@adaptiveTypes/index.ts"
...
const isAuthenticated = (_op?: unknown): OperationFunction<boolean> =>
  (_arg: unknown, localValues?: LocalValues): Promise<Either<Array<AdaptiveError>, boolean>> => {
    const user = localValues && (localValues as Record<string, unknown>)["user"]
    if (user) return Promise.resolve({ right: true })
    return Promise.resolve({ left: [Error("IsAuthenticated")("Authorize")("User is not authenticated.")] })
  }
