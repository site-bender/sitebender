...
Deno.test("getOperands multiple operations", async (t) => {
	...
	await t.step("should handle mixed operations with and without operands", () => {
		const operand1 = createMockInjector(1)
		const operand2 = createMockInjector(2)
		...
		const operation4 = createMockComparatorWithOperands([createMockInjector(3)])
		
		const result = getOperands([operation1, operation2, operation3, operation4])
...
Deno.test("getOperands with complex nested structures", async (t) => {
	await t.step("should handle nested operator operands", () => {
		...
		const nestedOperator = createMockOperatorWithOperands([innerOperand1, innerOperand2])
		
		const outerOperand = createMockInjector(3)
		const outerOperation = createMockOperatorWithOperands([nestedOperator, outerOperand])
	...
	await t.step("should handle mixed types of operands", () => {
		...
		const operatorOperand = createMockOperatorWithOperands([createMockInjector(10)])
		const comparatorOperand = createMockComparatorWithOperands([createMockInjector(15)])
...
Deno.test("getOperands realistic scenarios", async (t) => {
	await t.step("should handle mathematical expression scenario", () => {
		...
		const multiplyOperation = createMockOperatorWithOperands([addOperation, subtractOperation])
...
Deno.test("getOperands property-based tests", () => {
	...
	fc.assert(fc.property(
		fc.array(fc.array(fc.integer({ min: 1, max: 20 })), { minLength: 1, maxLength: 10 }),
		(operandGroups) => {
			...
			const result = getOperands(operations)
			const expectedLength = operandGroups.reduce((sum, group) => sum + group.length, 0)
	...
	fc.assert(fc.property(
		...
		(withOperandsCount, withoutOperandsCount) => {
			const operationsWithOperands = Array.from({ length: withOperandsCount }, () => 
				createMockOperatorWithOperands([createMockInjector(1)])
			)
			const operationsWithoutOperands = Array.from({ length: withoutOperandsCount }, () => 
				createMockOperatorWithoutOperands()
			)
			
			const allOperations = [...operationsWithOperands, ...operationsWithoutOperands]
...
Deno.test("getOperands performance characteristics", async (t) => {
	...
	await t.step("should handle operations with many operands efficiently", () => {
		const manyOperands = Array.from({ length: 1000 }, (_, i) => createMockInjector(i))
		...
		console.log(`Processing 1 operation with 1000 operands took ${end - start} milliseconds`)
	})
})