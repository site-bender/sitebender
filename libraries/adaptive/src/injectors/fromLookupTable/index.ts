import type { LocalValues, Either, AdaptiveError } from "../../../types/index.ts"
...
type LookupOp = { tag: string; datatype?: string; column: unknown; test: unknown }

const fromLookupTable = (op: LookupOp) =>
async (arg: unknown, localValues?: LocalValues): Promise<Either<Array<AdaptiveError>, unknown>> => {
	...
	const table = castValue(datatype)(getValue(op as unknown as { source: unknown })(localValues))
	...
	const out = (rows as Array<Record<string | number, unknown>>).reduce((acc, row) => {
		const val = row[col]
