...
import type { AsideAttributes } from "@adaptiveSrc/constructors/elements/types/attributes/index.ts"
...
import { ASIDE_ROLES } from "@adaptiveSrc/constructors/elements/constants/aria-roles.ts"
 ...
 */
export const filterAttributes = (attributes: AsideElementAttributes) => {
	const {
		id,
		// ARIA attributes (using general AriaAttributes since there's no specific AsideAriaAttributes)
		role,
	...
	if (isDefined(role)) {
		Object.assign(
			filteredAttrs,
			filterAttribute(isMemberOf(ASIDE_ROLES))("role")(role),
		)
	}
