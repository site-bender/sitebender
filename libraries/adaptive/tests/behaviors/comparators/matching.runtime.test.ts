...
const cmp3 = (cmp: string, a: unknown, b: unknown, c?: unknown): ComparatorNode => ({
  v: "0.1.0",
  kind: "comparator",
  id: nodeId(),
  cmp,
  args: [constInjector(a), constInjector(b), ...(c !== undefined ? [constInjector(c)] : [])],
})
...
Deno.test("Matches/DoesNotMatch runtime with invalid regex flags/patterns", async () => {
  ...
  const nm1 = await evaluateNode(cmp3("DoesNotMatch", "abc", "a.c", "invalid"), ctx)
