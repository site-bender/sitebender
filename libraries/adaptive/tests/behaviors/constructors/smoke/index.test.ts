...
import type { ComparatorConfig, LogicalConfig, Operand, Value, FromLookupInjector, FromLookupTableInjector, FromQueryStringInjector, FromUrlParameterInjector, FromApiInjector } from "@adaptiveTypes/index.ts"
...
const constant = (value: Value): Operand => {
  const dt = typeof value === "number" ? "Number" : typeof value === "boolean" ? "Boolean" : "String"
...
const andNode = (children: Array<ComparatorConfig | LogicalConfig>): LogicalConfig => ({
  tag: "And",
  type: "logical",
  datatype: "Boolean",
  operands: children,
...
const orNode = (children: Array<ComparatorConfig | LogicalConfig>): LogicalConfig => ({
  tag: "Or",
  type: "logical",
  datatype: "Boolean",
  operands: children,
...
describe("constructors - smoke", () => {
  ...
  it("FromLookupTable constructs config with source and test", () => {
    ...
    const inj: FromLookupTableInjector = FromLookupTable("Json")({ tableName: "prices", column: "usd", test: testOperand })
  ...
  it("FromUrlParameter constructs with segment", () => {
    const inj: FromUrlParameterInjector = FromUrlParameter("String")({ segment: 2 })
  ...
  it("FromApi constructs with endpoint and headers", () => {
    const inj: FromApiInjector = FromApi("Json")({ endpoint: "/api/x", method: "GET", headers: { A: "B" } })
