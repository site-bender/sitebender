...
import type { ActionNode, EventBindingNode, InjectorNode, IrDocument } from "../../../../types/ir/index.ts"
...
describe("hydrate - submit sets query string via Act.SetQueryString", () => {
      ...
      children: [
        {
          ...
          handler: {
            ...
            args: [
              { v: "0.1.0", kind: "injector", id: "k", injector: "From.Constant", datatype: "String", args: { value: "k" } } as InjectorNode,
              { v: "0.1.0", kind: "injector", id: "v", injector: "From.Constant", datatype: "String", args: { value: "v" } } as InjectorNode,
            ],
          } as ActionNode,
        } as EventBindingNode,
      ],
  ...
  // Provide globals for hydrate + action
  // deno-lint-ignore no-explicit-any
  ;(globalThis as any).document = document as unknown as Document
    // Capture replaceState calls
    const calls: Array<{ url: string }> = []
    // deno-lint-ignore no-explicit-any
    ;(globalThis as any).history = { replaceState: (_s: unknown, _t: string, url: string) => { calls.push({ url }) } }
    // deno-lint-ignore no-explicit-any
