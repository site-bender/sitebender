...
import type { IrDocument, EventBindingNode, ActionNode, InjectorNode } from "../../../../types/ir/index.ts"

function embedIr(doc: IrDocument) {
  return `<script id="ir-root" type="application/adaptive+json">${JSON.stringify(doc)}</script>`
}
...
describe("hydrate - binds event and dispatches action", () => {
  it("click handler updates text via Act.SetValue", async () => {
    const irDoc: IrDocument = {
      ...
      attrs: {},
      children: [
        { v: "0.1.0", kind: "on", id: "h1", event: "On.Click", handler: { v: "0.1.0", kind: "action", id: "a1", action: "Act.SetValue", args: [
          { v: "0.1.0", kind: "injector", id: "sel", injector: "From.Constant", datatype: "String", args: { value: "#target" } } as InjectorNode,
          { v: "0.1.0", kind: "injector", id: "val", injector: "From.Constant", datatype: "String", args: { value: "new" } } as InjectorNode,
        ] } as ActionNode } as EventBindingNode,
      ],
    }
    ...
    // @ts-ignore minimal history/location for SetQueryString if used
    ...
    // deno-lint-ignore no-explicit-any
    const { registerEvent } = await import("../../../../src/operations/registries/events.ts") as any
    registerEvent("On.Click", (el: HTMLElement, _node: EventBindingNode, dispatch: () => void) => {
