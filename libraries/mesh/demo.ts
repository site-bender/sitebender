...
/**
 * @sitebender/mesh Demo
 *
 * This demo showcases the core features of the mesh library:
...
function demoStorage() {
	...
	console.log("\nLoading data...")
	storage.load("my-register")

	console.log("\nDeleting data...")
	storage.delete("my-register")
}
...
function demoSync() {
	...
	const _sync = createStateSyncProtocol(crdt, mockTransport, 5000)
...
║        @sitebender/mesh Library Demo          ║
╚════════════════════════════════════════════╝${colors.reset}

${colors.dim}A functional, offline-first mesh networking library
...
${colors.cyan}tests/unit/mesh/${colors.reset}

${colors.dim}Or read the source code in:${colors.reset}
${colors.cyan}libraries/mesh/src/${colors.reset}
`)
}
