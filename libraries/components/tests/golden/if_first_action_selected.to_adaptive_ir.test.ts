...
import type { IrDocument, ActionNode } from "../../../adaptive/types/ir/index.ts"
...
Deno.test("golden: Act.If selects the first action in each branch", () => {
  const tree = [
    ...
    On({
      ...
      children: If({
        children: [
          Condition({ children: IsEqualTo({ type: "String", children: [
  ...
  const thenPayload = (thenA.args?.[0] as { args?: { value?: string } })?.args?.value
  const elsePayload = (elseA.args?.[0] as { args?: { value?: string } })?.args?.value
