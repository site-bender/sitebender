...
import type { IrDocument, ActionNode, ComparatorNode } from "../../../adaptive/types/ir/index.ts"
  ...
  const tree = [
    ...
    On({
      ...
      children: If({
        children: [
          Condition({
            children: IsEqualTo({ type: "String", children: [
              Constant({ value: "foo" }) as unknown as JSX.Element,
              Constant({ value: "foo" }) as unknown as JSX.Element,
            ] }) as unknown as JSX.Element,
          }) as unknown as JSX.Element,
          IfTrue({ children: Publish({ topic: "then" }) as unknown as JSX.Element }) as unknown as JSX.Element,
          IfFalse({ children: Publish({ topic: "else" }) as unknown as JSX.Element }) as unknown as JSX.Element,
        ],
      }) as unknown as JSX.Element,
    }),
  ]
