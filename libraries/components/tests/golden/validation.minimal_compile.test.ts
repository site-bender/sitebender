...
Deno.test("golden: Validation attaches to prior element in minimal compiler", () => {
  const tree = [
    ...
    Validation({
      when: "input",
      children: NotEmpty({ children: FromElement({ id: "email" }) as unknown as JSX.Element }) as unknown as JSX.Element,
    }),
  ]
  ...
  const v = (elNode.behaviors || [])[0] as { kind?: string; when?: string; rule?: unknown }
