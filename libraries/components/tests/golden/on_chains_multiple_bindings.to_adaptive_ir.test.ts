...
import type { IrDocument, EventBindingNode } from "../../../adaptive/types/ir/index.ts"
...
Deno.test("golden: multiple On markers create multiple event bindings (chains)", () => {
  const tree = [
    ...
    On({ event: "Input", children: Publish({ topic: "first" }) as unknown as JSX.Element }),
    On({ event: "Change", children: Publish({ topic: "second" }) as unknown as JSX.Element }),
    { type: "input", props: { id: "b" } },
    On({ event: "Input", children: Publish({ topic: "third" }) as unknown as JSX.Element }),
  ]
