...
import type { IrDocument, ActionNode, ComparatorNode, InjectorNode } from "../../../adaptive/types/ir/index.ts"
...
Deno.test("golden: If with Matches condition compiles comparator with pattern and flags", () => {
  const condition = Matches({ children: [
    ...
    On({
      ...
      children: If({
        children: [
          ...
          IfTrue({ children: Publish({ topic: "ok" }) as unknown as JSX.Element }) as unknown as JSX.Element,
          IfFalse({ children: Publish({ topic: "nope" }) as unknown as JSX.Element }) as unknown as JSX.Element,
        ],
      }) as unknown as JSX.Element,
    }),
  ]
