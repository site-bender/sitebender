  ...
  const tree = [
    ...
    On({
      ...
      children: Publish({
        ...
        payload: Matches({ children: [FromElement({ id: "val" }) as unknown as JSX.Element] }) as unknown as JSX.Element,
      }) as unknown as JSX.Element,
    }),
  ]
  const doc = compileToAdaptive(tree) as IrDocument
...
Deno.test("compiler emits warning for Subtract arity not equal to 2", () => {
  const tree = [
    ...
    On({
      ...
      children: Publish({
        topic: "debug",
        payload: Subtract({ children: [FromElement({ id: "a" }) as unknown as JSX.Element] }) as unknown as JSX.Element,
      }) as unknown as JSX.Element,
    }),
  ]
  const doc = compileToAdaptive(tree) as IrDocument
...
Deno.test("compiler emits warning for Divide arity not equal to 2", () => {
  const tree = [
    ...
    On({
      ...
      children: Publish({
        topic: "debug",
        payload: Divide({ children: [FromElement({ id: "a" }) as unknown as JSX.Element] }) as unknown as JSX.Element,
      }) as unknown as JSX.Element,
    }),
  ]
  const doc = compileToAdaptive(tree) as IrDocument
