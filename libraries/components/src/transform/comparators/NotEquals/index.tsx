...
export default function NotEquals({ left, right, children }: Props): ComparatorMarker {
  const kids = Array.isArray(children) ? children : (children ? [children] : [])
  const l = left ?? kids[0]
  const r = right ?? kids[1]
  return { __kind: "comparator", cmp: "Is.UnequalTo", args: [l, r].filter((x) => typeof x !== "undefined") }
}
