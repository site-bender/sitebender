...
export type IRNode =
  | { kind: "element"; tag: string; props: Record<string, unknown>; children: IRNode[]; behaviors?: BehaviorNode[] }
  | { kind: "text"; value: string }

export type BehaviorNode =
  | { kind: "conditional"; condition?: unknown; ifTrue: IRNode[]; ifFalse: IRNode[] }
...
const isValidation = (
  x: unknown,
): x is { __kind: "control:validation"; when: "input" | "blur" | "submit"; rule?: unknown } =>
  getKind(x) === "control:validation"

const isConditional = (
  x: unknown,
): x is { __kind: "control:conditional"; condition?: unknown; ifTrue?: unknown; ifFalse?: unknown } =>
  getKind(x) === "control:conditional"

export default function compile(children?: MaybeVNode | MaybeVNode[]): IRNode[] {
  ...
  for (const node of arr) {
    ...
    if (isValidation(node)) {
      ...
      const behavior: BehaviorNode = { kind: "validation", when: node.when, rule: node.rule }
    ...
    if (isConditional(node)) {
      const condNode = node as { __kind: "control:conditional"; condition?: unknown; ifTrue?: unknown; ifFalse?: unknown }
    ...
    if (isObject(node) && "type" in node) {
      ...
      const type = v.type
      const props = v.props ?? {}
      const tag = typeof type === "string" ? type : (isObject(type) && "name" in type && typeof (type as { name?: unknown }).name === "string") ? (type as { name?: string }).name ?? "div" : "div"
      const childIR = compile((props as Record<string, unknown>).children as MaybeVNode | MaybeVNode[])
