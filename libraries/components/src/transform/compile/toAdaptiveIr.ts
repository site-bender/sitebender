...
const metaWithWarnings = (warnings: string[] | undefined) =>
  (warnings && warnings.length > 0)
    ? ({ meta: { debug: { warnings } } } as { meta: { debug: Record<string, unknown> } })
    : ({})
...
const isMinCtor = (x: unknown): x is MinCtor => isCtor(x) && (x as { tag: string }).tag === "Min" && (x as { type: string }).type === "operator"
const isMaxCtor = (x: unknown): x is MaxCtor => isCtor(x) && (x as { tag: string }).tag === "Max" && (x as { type: string }).type === "operator"
...
function compileOperand(x: unknown): Node {
  ...
  if (isComparatorMarker(x)) {
    ...
    if (cmpTag === "Is.EqualTo" || cmpTag === "Is.UnequalTo") {
      if (argCount !== 2) warnings.push(`${cmpTag} expects 2 arguments, got ${argCount}`)
    }
    if (cmpTag === "Is.And" || cmpTag === "Is.Or") {
      if (argCount < 1) warnings.push(`${cmpTag} expects at least 1 argument, got ${argCount}`)
    }
    if (cmpTag === "Is.NotEmpty") {
      if (argCount !== 1) warnings.push(`${cmpTag} expects 1 argument, got ${argCount}`)
    }
    return {
      v: "0.1.0",
      kind: "comparator",
  ...
  if (isCtor(x) && (x as { type: string }).type === "operator" && (x as { tag: string }).tag === "Subtract") {
    const datatype = normalizeDatatype((x as { datatype?: unknown }).datatype) ?? "Float"
  ...
  if (isCtor(x) && (x as { type: string }).type === "operator" && (x as { tag: string }).tag === "Divide") {
    const datatype = normalizeDatatype((x as { datatype?: unknown }).datatype) ?? "Float"
  ...
  if (isLogicalCtor(x)) {
    ...
    const ops = Array.isArray(x.operands) ? x.operands : []
    const warnings: string[] = []
    if ((tag === "Is.And" || tag === "Is.Or") && ops.length < 1) warnings.push(`${tag} expects at least 1 operand`)
    return {
      v: "0.1.0",
      kind: "comparator",
      id: nextC(),
  ...
  const warnings: string[] = []
    ...
    if (x.tag === "Matches" || x.tag === "DoesNotMatch") {
      ...
      if (rawCount < 2) warnings.push(`${tag} expects 2 or 3 arguments (operand, pattern, flags?), got ${rawCount}`)
    }
    ...
    if (x.tag === "InSet") {
      ...
      if (rawCount !== 2) warnings.push(`InSet expects 2 arguments (value, set), got ${rawCount}`)
    }
  ...
  if ((cmp === "Matches" || cmp === "Is.Matches") && (args.length < 2 || args.length > 3)) {
      warnings.push(`${cmp} expects 2 or 3 arguments (operand, pattern, flags?), got ${args.length}`)
    }
    if ((cmp === "DoesNotMatch" || cmp === "Is.DoesNotMatch") && (args.length < 2 || args.length > 3)) {
      warnings.push(`${cmp} expects 2 or 3 arguments (operand, pattern, flags?), got ${args.length}`)
    }
    return {
      v: "0.1.0",
