...
type SlotMarker = { __kind: "slot:condition" | "slot:iftrue" | "slot:iffalse"; children?: JSX.Element | Array<JSX.Element> }

const toArray = (x?: JSX.Element | Array<JSX.Element>): Array<JSX.Element> =>
  x ? (Array.isArray(x) ? x : [x]) : []

export default function If({ condition, then: thenBranch, else: elseBranch, children }: Props): ConditionalMarker {
  // If slots exist in children, prefer them.
  const arr = (Array.isArray(children) ? children : (children ? [children] : [])) as Array<unknown>
  ...
  if (ifTrue.length === 0 && ifFalse.length === 0 && arr.length > 0 && slots.length === 0) {
    const first = arr[0] as JSX.Element
    const second = arr[1] as JSX.Element | undefined
    return { __kind: "control:conditional", condition: cond, ifTrue: toArray(first), ifFalse: toArray(second) }
  }
