...
export default function Conditional({ children }: Props): ConditionalMarker {
  const rawChildren: Array<unknown> = Array.isArray(children) ? [...children] : (children ? [children] : [])
  ...
  const isSlot = (x: unknown): x is SlotMarker => {
    if (x && typeof x === "object") {
      ...
      if (typeof maybe.__kind === "string") {
        ...
        return k === "slot:condition" || k === "slot:iftrue" || k === "slot:iffalse"
      }
    }
    return false
  }
  ...
  const toArray = (c?: JSX.Element | Array<JSX.Element>) => c ? (Array.isArray(c) ? c : [c]) : []
