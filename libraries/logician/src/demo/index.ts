...
/**
 * Demo: Proving the Logician Works
 *
 * This demo shows that the logician ACTUALLY generates real, working tests
 * by:
...
export default async function runDemo() {
	...
	console.log(`${BOLD}${CYAN}   🧪 LOGICIAN DEMONSTRATION 🧪${RESET}`)
	console.log(
		`${BOLD}${CYAN}========================================${RESET}\n`,
	)

	console.log(`${YELLOW}This demo will prove that the logician:${RESET}`)
	...
	const projectRoot = baseDir.replace("/libraries/logician/src/demo/", "")
	...
	const testTargets = [
		...
		{
			path: `${projectRoot}/libraries/toolsmith/src/vanilla/array/head/index.ts`,
	...
	console.log(`${GREEN}The logician has demonstrated:${RESET}`)
	...
	console.log(`\n${CYAN}To use the logician on your own functions:${RESET}`)
	console.log(
		`  ${BLUE}import generateTests from "@sitebender/logician"${RESET}`,
	)
	console.log(
