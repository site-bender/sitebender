...
Deno.test("combineValidations - returns last valid when any are valid; accumulates errors otherwise", async (t) => {
  ...
  await t.step("accumulates all errors when none valid", () => {
    const errs1: NonEmptyArray<ValidationError> = [{ field: "a", messages: ["e1"] }]
    const errs2: NonEmptyArray<ValidationError> = [{ field: "b", messages: ["e2"] }]

    const vals: NonEmptyArray<ReturnType<typeof invalid<ValidationError, number>>> = [
