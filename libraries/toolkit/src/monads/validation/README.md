...
Minimal, composable validation primitives with great TypeScript inference. This folder is the reference for style, tests, and docs patterns across the toolkit.
...
This is a temporary measure. Once the Prover library is ready, it will synthesize and maintain the full test suites for the Toolkit and the rest of the codebase (including itself). The plan is to remove all handwritten tests (unit, property, integration) in favor of Prover-generated tests. Any remaining manual tests would serve only as supplemental safety harnesses around the generated proofs/tests.
