...
Deno.test("inRange: basic functionality", async (t) => {
	...
	await t.step("large numbers", () => {
		...
		assertEquals(inRange(Number.MIN_SAFE_INTEGER)(Number.MAX_SAFE_INTEGER)(0), true)
	...
	fc.assert(
		fc.property(
			...
			(a, b, offset) => {
				...
				assertEquals(inRange(a)(b)(belowRange), false, 
					`Expected ${belowRange} to be outside [${min}, ${max})`)
				
				// Test value above range
				const aboveRange = max + Math.abs(offset)
				assertEquals(inRange(a)(b)(aboveRange), false,
