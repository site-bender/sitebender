...
Deno.test("add - returns NaN for non-number inputs", () => {
	...
	for (const invalid of invalidInputs) {
		assertEquals(Number.isNaN(add(invalid as any)(5)), true, `Failed for ${String(invalid)}`)
		assertEquals(Number.isNaN(add(5)(invalid as any)), true, `Failed for ${String(invalid)}`)
	}
})
