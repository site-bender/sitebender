	...
	fc.assert(
		fc.property(fc.integer(), (n) => {
			...
			return JSON.stringify(res) ===
				JSON.stringify(ok<number, string>(double(n)))
		}),
		{ numRuns: 200 },
	)
...
Deno.test("Result fold collapses Ok/Err with provided handlers", () => {
	...
	fc.assert(
		fc.property(
			fc.integer(),
			(n) => fold(onErr)(onOk)(ok<number, string>(n)) === `V:${n}`,
		),
		{ numRuns: 200 },
	)
	...
	fc.assert(
		fc.property(
			fc.string(),
			(e) => fold(onErr)(onOk)(err<string, number>(e)) === `E:${e}`,
		),
		{ numRuns: 200 },
	)
})
...
Deno.test("Result orElse recovers from Err only", () => {
	...
	fc.assert(
		fc.property(fc.string(), (e) => {
			...
			return JSON.stringify(res) ===
				JSON.stringify(ok<number, string>(e.length))
		}),
		{ numRuns: 200 },
	)
