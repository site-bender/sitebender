...
Deno.test("sort: JSDoc examples", async (t) => {
	...
	await t.step("sorts [1, 2, 3] to [3, 2, 1] with descending comparator", () => {
	...
	await t.step("sorts objects by age property", () => {
		const byAge = sort((a: {age: number}, b: {age: number}) => a.age - b.age)
		assertEquals(
...
Deno.test("sort: edge cases", async (t) => {
	...
	await t.step("handles negative numbers", () => {
		assertEquals(sort((a: number, b: number) => a - b)([-3, -1, -2]), [-3, -2, -1])
	})

	await t.step("handles mixed positive and negative", () => {
		assertEquals(sort((a: number, b: number) => a - b)([3, -1, 0, -2, 1]), [-2, -1, 0, 1, 3])
	})
	...
	await t.step("handles strings with proper numeric sort", () => {
		assertEquals(
			sort((a: string, b: string) => parseInt(a) - parseInt(b))(["10", "2", "1"]),
			["1", "2", "10"]
		)
	})
	...
	await t.step("handles case-insensitive string sort", () => {
		assertEquals(
			sort((a: string, b: string) => a.toLowerCase().localeCompare(b.toLowerCase()))(["b", "A", "c", "B"]),
			["A", "b", "B", "c"]
		)
	})
})
...
Deno.test("sort: complex sorting", async (t) => {
	...
	await t.step("stable sort for equal elements", () => {
		...
		const sorted = sort((a: Item, b: Item) => a.value.localeCompare(b.value))(data)
