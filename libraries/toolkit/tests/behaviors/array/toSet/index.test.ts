...
Deno.test("toSet: dates", () => {
	...
	assertEquals(
		dateSet.size,
		2,
		"should have 2 date objects (different references)",
	)
		...
		fc.property(
			...
			(arr) => {
				...
				assertEquals(
					set.size,
					setAgain.size,
					"Size should be same after round-trip",
				)
			},
		),
		{ numRuns: 100 },
...
Deno.test("toSet: practical use cases", () => {
	...
	assertEquals(
		countUnique(["a", "b", "a", "c", "b"]),
		3,
		"should count unique strings",
	)
