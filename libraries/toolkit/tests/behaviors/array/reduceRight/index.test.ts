...
Deno.test("reduceRight: basic functionality", async (t) => {
	...
	await t.step("should pass correct arguments to reducer", () => {
		...
		const spy = (acc: unknown, elem: unknown, idx: number, arr: ReadonlyArray<unknown>) => {
...
Deno.test("reduceRight: comparison with reduce", async (t) => {
	await t.step("should produce different results than reduce for non-commutative operations", () => {
	...
	await t.step("should produce same results as reduce for commutative operations", () => {
		const add = (acc: number, x: number) => acc + x
...
Deno.test("reduceRight: practical examples", async (t) => {
	await t.step("should compose functions from right to left", () => {
		...
		const composed = reduceRight(compose)((x: number) => x)([double, addOne, square])
	...
	await t.step("should build tree structure from path", () => {
		...
		const buildPath = (tree: Tree, segment: string): Tree => ({ [segment]: tree })
		
		const result = reduceRight(buildPath)({} as Tree)(["home", "user", "documents"])
		assertEquals(result, {
			home: {
				user: {
					documents: {},
...
Deno.test("reduceRight: property-based tests", async (t) => {
	...
	await t.step("should preserve sum regardless of direction for addition", () => {
		fc.assert(
			fc.property(
