...
Deno.test("takeLastWhile: string predicates", () => {
	...
	assertEquals(
		takeLastWhile((s: string) => s.length > 3)(["a", "bb", "ccc", "dddd", "eeeee"]),
		["dddd", "eeeee"],
		"should take trailing strings with length > 3"
	)

	assertEquals(
		takeLastWhile((s: string) => s.includes("x"))(["ax", "bx", "cy", "dx", "ex"]),
		["dx", "ex"],
...
Deno.test("takeLastWhile: predicate with index and array", () => {
	...
	assertEquals(
		takeLastWhile((x: number, i: number, arr: ReadonlyArray<number>) => x > arr[0])(
			[3, 1, 2, 4, 5, 6]
		),
		[4, 5, 6],
...
Deno.test("takeLastWhile: immutability", () => {
	...
	assertEquals(allMatch === original, false, "should be different array instance")
})
		...
		fc.property(
			...
			(arr, predicate) => {
				...
				if (result.length === 0) {
					...
					const isSuffix = arr.slice(-result.length).every((v, i) => v === result[i])
	...
	fc.assert(
		fc.property(
			...
			(arr) => {
				...
				assertEquals(result, arr, "should return all elements when predicate is always true")
			}
		),
		{ numRuns: 100 }
	)
	...
	fc.assert(
		fc.property(
			...
			(arr) => {
				...
				assertEquals(result, [], "should return empty when predicate is always false")
			}
		),
		{ numRuns: 100 }
	)
...
Deno.test("takeLastWhile: performance considerations", () => {
	...
	assertEquals(result2, [], "should terminate early when predicate fails immediately")
})
