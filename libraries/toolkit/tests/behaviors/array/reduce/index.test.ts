...
Deno.test("reduce - build new array (map)", () => {
	const result = reduce((acc: Array<number>, n: number) => [...acc, n * 2])([])([1, 2, 3])
	assertEquals(result, [2, 4, 6])
})

Deno.test("reduce - filter", () => {
	const result = reduce((acc: Array<number>, n: number) => n > 2 ? [...acc, n] : acc)([])([1, 2, 3, 4])
	assertEquals(result, [3, 4])
})

Deno.test("reduce - build object", () => {
	const result = reduce((acc: Record<string, number>, [k, v]: [string, number]) => ({...acc, [k]: v}))({})([["a", 1], ["b", 2]])
	assertEquals(result, {a: 1, b: 2})
})

Deno.test("reduce - flatten", () => {
	const result = reduce((acc: Array<number>, arr: Array<number>) => acc.concat(arr))([])([[1, 2], [3, 4]])
...
Deno.test("reduce - find maximum", () => {
	const result = reduce((acc: number, n: number) => Math.max(acc, n))(-Infinity)([3, 1, 4, 1, 5])
	assertEquals(result, 5)
})

Deno.test("reduce - find minimum", () => {
	const result = reduce((acc: number, n: number) => Math.min(acc, n))(Infinity)([3, 1, 4, 1, 5])
...
Deno.test("reduce - reverse array", () => {
	const result = reduce((acc: Array<number>, n: number) => [n, ...acc])([])([1, 2, 3, 4])
...
Deno.test("reduce - group by property", () => {
	...
	const result = reduce((acc: Record<number, Array<Person>>, person: Person) => {
...
Deno.test("reduce - string concatenation", () => {
	const result = reduce((acc: string, s: string) => acc + s)("")(["Hello", " ", "World"])
...
Deno.test("reduce - handles sparse arrays", () => {
	...
	const result = reduce((acc: Array<number | undefined>, n: number | undefined) => [...acc, n])([])
...
Deno.test("reduce - NaN and Infinity", () => {
	const result = reduce((acc: number, n: number) => acc + n)(0)([NaN, Infinity, -Infinity, 1])
...
Deno.test("reduce - builds same array when used as identity", () => {
	fc.assert(
		fc.property(
			...
			(array) => {
				const rebuilt = reduce((acc: Array<any>, item: any) => [...acc, item])([])(array)
...
Deno.test("reduce - different initial types", () => {
	...
	// Initial as boolean
	const asBoolean = reduce((acc: boolean, n: number) => acc && n > 0)(true)(numbers)
