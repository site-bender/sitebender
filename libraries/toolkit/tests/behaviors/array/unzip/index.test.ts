...
Deno.test("unzip: table data extraction", () => {
	...
	assertEquals(products, ["Product A", "Product B", "Product C"], "should extract product names")
	assertEquals(prices, [100, 200, 150], "should extract prices")
})

Deno.test("unzip: empty array", () => {
	const result = unzip([])
	assertEquals(result, [[], []], "should return two empty arrays for empty input")
	...
	assertEquals(Array.isArray(result[0]), true, "first element should be an array")
	assertEquals(Array.isArray(result[1]), true, "second element should be an array")
})
...
Deno.test("unzip: mixed types", () => {
	...
	assertEquals(first, [1, "two", 3, "four"], "should handle mixed first elements")
	assertEquals(second, [true, false, null, true], "should handle mixed second elements")
})
...
Deno.test("unzip: nested arrays", () => {
	...
	assertEquals(numbers, [[1, 2], [3, 4], [5, 6]], "should handle nested number arrays")
	assertEquals(strings, [["a", "b"], ["c", "d"], ["e", "f"]], "should handle nested string arrays")
})
...
Deno.test("unzip: malformed input handling", () => {
	...
	assertEquals(first, [1, 4, 6], "should skip malformed pairs (missing elements)")
...
Deno.test("unzip: immutability", () => {
	...
	assertEquals(original, [[1, "a"], [2, "b"], [3, "c"]], "original should not be modified")
	
	// Result should be modified
	assertEquals(result, [[1, 2, 3, 4], ["a", "b", "c", "d"]], "result should be modified")
})
...
Deno.test("unzip: property-based tests", () => {
	...
	fc.assert(
		fc.property(
			...
			(pairs) => {
				const [first, second] = unzip(pairs)
				assertEquals(first.length, pairs.length, "first array length should match input")
				assertEquals(second.length, pairs.length, "second array length should match input")
			}
		),
		{ numRuns: 100 }
	)
	...
	fc.assert(
		fc.property(
			...
			(empty) => {
				...
				assertEquals(result, [[], []], "empty input should produce empty arrays")
			}
		),
		{ numRuns: 1 }
	)
	...
	fc.assert(
		fc.property(
			...
			(arr1, arr2) => {
				...
				const pairs: Array<[number, string]> = trimmed1.map((v, i) => [v, trimmed2[i]])
...
Deno.test("unzip: real-world scenarios", () => {
	...
	assertEquals(studentIds, ["STU001", "STU002", "STU003", "STU004"], "should extract student IDs")
	assertEquals(scores, [85, 92, 78, 95], "should extract scores")
	
	// Statistical mean calculation
	const mean = (nums: Array<number>) => nums.reduce((a, b) => a + b, 0) / nums.length
