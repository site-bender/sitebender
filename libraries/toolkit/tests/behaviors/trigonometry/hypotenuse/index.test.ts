...
Deno.test("hypotenuse - large values (avoids overflow)", () => {
	...
	const large = 1e200
	assertAlmostEquals(hypotenuse([large, large]), Math.sqrt(2) * large, large * 1e-10)
	
	// 3D large values
	assertAlmostEquals(hypotenuse([large, large, large]), Math.sqrt(3) * large, large * 1e-10)
})

Deno.test("hypotenuse - small values (maintains precision)", () => {
	// Should handle small values without underflow
	const small = 1e-200
	assertAlmostEquals(hypotenuse([small, small]), Math.sqrt(2) * small, small * 1e-10)
	
	// 3D small values
	assertAlmostEquals(hypotenuse([small, small, small]), Math.sqrt(3) * small, small * 1e-10)
})
...
Deno.test("hypotenuse - practical examples", () => {
	...
	assertAlmostEquals(hypotenuse([screenWidth, screenHeight]), Math.sqrt(1920*1920 + 1080*1080), 0.1)
...
Deno.test("hypotenuse - equivalence to Math.hypot", () => {
	fc.assert(
		fc.property(
			fc.array(fc.float({ noNaN: true, min: Math.fround(-1000), max: Math.fround(1000) }), 
