...
Deno.test("logarithm: practical applications", async (t) => {
	...
	await t.step("compound interest periods", () => {
		...
		assertEquals(Math.abs(periodsToDouble(0.01) - 69.66071689357483) < 1e-8, true) // ~70 years at 1%
		assertEquals(Math.abs(periodsToDouble(0.05) - 14.206699082890463) < 1e-8, true) // ~14 years at 5%
		assertEquals(Math.abs(periodsToDouble(0.10) - 7.272540897341715) < 1e-8, true) // ~7 years at 10%
		assertEquals(Math.abs(periodsToDouble(0.15) - 4.959484454764117) < 1e-8, true) // ~5 years at 15%
	})
