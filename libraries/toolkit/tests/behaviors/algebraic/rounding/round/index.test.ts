...
Deno.test("round", async (t) => {
	await t.step("rounding properties", async (t) => {
		...
		await t.step("should round to nearest integer with minimum distance", () => {
			fc.assert(
				fc.property(
	...
	await t.step("special values", async (t) => {
		...
		await t.step("should handle large numbers", () => {
			...
			assertEquals(round(Number.MAX_SAFE_INTEGER - 0.4), Number.MAX_SAFE_INTEGER)
			assertEquals(round(Number.MIN_SAFE_INTEGER + 0.4), Number.MIN_SAFE_INTEGER)
		})
