...
Deno.test("exponential: special values", async (t) => {
	...
	await t.step("very large positive values", () => {
		...
		assertEquals(approximately(exponential(500), 1.4035922178528374e+217, 1e+207), true)
		assertEquals(approximately(exponential(700), 1.0142320547350045e+304, 1e+294), true)
...
Deno.test("exponential: practical applications", async (t) => {
	await t.step("continuous compound interest", () => {
		...
		assertEquals(approximately(continuousGrowth(0.05, 10), 1.6487212707001282), true)
		
		// 10% annual rate for 5 years
		assertEquals(approximately(continuousGrowth(0.10, 5), 1.6487212707001282), true)
		
		// 3% annual rate for 20 years
		assertEquals(approximately(continuousGrowth(0.03, 20), 1.8221188003905092), true)
	})
	...
	await t.step("population growth", () => {
		...
		assertEquals(approximately(population(1000, 0.02, 25), 1648.7212707001282), true)
		
		// 3% growth rate for 10 years
		assertEquals(approximately(population(5000, 0.03, 10), 6749.293525605318, 0.01), true)
		
		// Negative growth (decline)
		assertEquals(approximately(population(10000, -0.01, 20), 8187.307530779819), true)
	})
	...
	await t.step("probability distributions", () => {
		...
		// Mean = 1/λ
		assertEquals(approximately(exponentialPDF(2, 0.5), 2 * exponential(-1)), true)
		assertEquals(approximately(exponentialPDF(0.5, 2), 0.5 * exponential(-1)), true)
		...
		assertEquals(approximately(exponentialCDF(2, 0.5), 1 - exponential(-1)), true)
	})
})
...
Deno.test("exponential: relationships with other functions", async (t) => {
	...
	await t.step("Euler's identity components", () => {
		...
		assertEquals(approximately(exponential(-Math.PI), 0.04321391826377226), true)
	})
})