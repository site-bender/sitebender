...
Deno.test("add - associative property: (a + b) + c = a + (b + c)", () => {
	fc.assert(
		fc.property(finiteNumber(), finiteNumber(), finiteNumber(), (a, b, c) => {
			...
			const maxOutput = Math.max(Math.abs(leftAssociative), Math.abs(rightAssociative))
...
Deno.test("add - associative with special values", () => {
	...
	for (const [a, b, c] of testCases) {
		...
		const maxValue = Math.max(Math.abs(leftAssociative), Math.abs(rightAssociative))
		const epsilon = maxValue * 1e-10
		
		assertEquals(
			approximately(leftAssociative, rightAssociative, Math.max(epsilon, 1e-10)),
			true,
	...
	fc.assert(
		fc.property(
			...
			(numbers) => {
				...
				// ((a + b) + c) + d
				const grouping1 = rest.reduce((acc, n) => add(acc)(n), add(add(add(a)(b))(c))(d))
				
				// (a + b) + (c + d)
				const grouping2 = rest.reduce((acc, n) => add(acc)(n), add(add(a)(b))(add(c)(d)))
				
				// a + ((b + c) + d)
				const grouping3 = rest.reduce((acc, n) => add(acc)(n), add(a)(add(add(b)(c))(d)))
				
				// a + (b + (c + d))
				const grouping4 = rest.reduce((acc, n) => add(acc)(n), add(a)(add(b)(add(c)(d))))
