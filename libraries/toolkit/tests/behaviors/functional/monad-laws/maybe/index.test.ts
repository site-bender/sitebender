...
Deno.test("Maybe monad - associativity law", () => {
	...
	const g = (n: number): Maybe<number> => 
		n > 100 ? nothing() : just(n + 10)
...
Deno.test("Maybe - law interactions", () => {
	...
	const result1 = chain<number, number>(g)(chain<number, number>(f)(just(value)))
	const result2 = chain<number, number>((x: number) => chain<number, number>(g)(f(x)))(just(value))
