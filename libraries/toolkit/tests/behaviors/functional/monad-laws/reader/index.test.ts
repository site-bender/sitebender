...
Deno.test("Reader monad - left identity law", () => {
  ...
  const left = chain<number, number, number>((x: number) => (r: number) => f(x)(r))(m)
...
Deno.test("Reader monad - associativity law", () => {
  ...
  fc.assert(
    fc.property(fc.integer(), fc.integer(), (value, env) => {
      const m = of<number, number>(value)
      const left = chain<number, number, number>(g)(chain<number, number, number>(f)(m))
      const right = chain<number, number, number>((x: number) => chain<number, number, number>(g)(f(x)))(m)
      return left(env) === right(env)
    }),
    { numRuns: 300 },
  )
...
Deno.test("Reader behaviors - ask/asks/local", () => {
  ...
  const combined = chain<number, number, typeof env>((m: number) => (_: typeof env) => m + 1)(r3)

  // local to transform env
  const localR = local<typeof env, { multiplier: number; offset: number }>((e: { multiplier: number; offset: number }) => ({ ...e, offset: e.offset + 1 }))(r1)
