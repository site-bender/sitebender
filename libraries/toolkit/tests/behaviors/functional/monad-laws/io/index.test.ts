...
Deno.test("IOMaybe map/chain and IOEither map/chain preserve semantics", () => {
	...
	const m3 = chainIOMaybe((n: number) =>
		ioMaybe(() => (n > 0 ? just(n + 5) : nothing()))
	)(m2)
	const mv = runIO(m3)
	if (!isJust(mv) || mv.value !== 25) {
		throw new Error("IOMaybe map/chain failed")
	}

	const ee = ioEither(() => right<number, string>(10))
	const e2 = mapIOEither<string, number, number>((n: number) => n * 3)(ee)
	const e3 = chainIOEither<string, number, number>((n: number) =>
		ioEither(() => (n > 0 ? right(n + 1) : left("neg")))
	)(e2)
	const ev = runIO(e3)
	if (ev._tag !== "Right" || ev.right !== 31) {
		throw new Error("IOEither map/chain failed")
	}
})
...
Deno.test("IO to IOMaybe/IOEither helpers wrap values in Just/Right", () => {
	...
	const e = runIO(ioe)
	if (e._tag !== "Right" || e.right !== 7) {
		throw new Error("ioToIOEither failed")
	}
})
...
Deno.test("liftMaybe/liftEither execute pure fns in IO context", () => {
	...
	const ee2 = runIO(le)
	if (ee2._tag !== "Right" || ee2.right !== 99) {
		throw new Error("liftEither failed")
	}
})
