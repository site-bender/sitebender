...
Deno.test("State monad - left/right identity and associativity", () => {
  ...
  fc.assert(
    fc.property(fc.integer(), fc.integer(), (value, s0) => {
      ...
      const assocLeft = chain<number, number, number>(g)(chain<number, number, number>(f)(m))(env)
      const assocRight = chain<number, number, number>((x: number) => chain<number, number, number>(g)(f(x)))(m)(env)
...
Deno.test("State behaviors - get/put/modify/eval/exec", () => {
  ...
  const program: State<number, number> = chain<number, number, number>(() => inc)(get<number>())
  const res = program(10)
  if (JSON.stringify(res) !== JSON.stringify([11, 11])) throw new Error("state program failed")
