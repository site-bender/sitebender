//++ Classify a RawComment fullText into marker semantic bucket
import parseExample from "../parseExample/index.ts"
import stripBlock from "../stripBlock/index.ts"

export type ClassifiedMarker =
	| { kind: "description"; value: string }
	| {
		kind: "example"
		examples: Array<{ code: string; expected?: string; raw: string }>
	}
	| { kind: "techDebt"; reason: string }

export default function classifyMarker(
	full: string,
): ClassifiedMarker | undefined {
	...
	if (trimmed.startsWith("//??")) {
		return {
			kind: "example",
			examples: [parseExample(trimmed.slice(4).trim())],
		}
	}
	if (trimmed.startsWith("/*??")) {
