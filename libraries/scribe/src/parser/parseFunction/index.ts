import type { ASTNode, FunctionSignature, ParseError, Result } from "../../types/index.ts"
...
export default function parseFunction(
	...
	try {
		...
		return {
			ok: false,
			error: {
				message: error instanceof Error ? error.message : "Failed to parse function",
			},
		}
	}
}
 ...
 */
function extractParameters(source: string): Array<{ name: string; type: string; optional: boolean }> {
 ...
 */
function extractGenerics(source: string): Array<{ name: string; constraint?: string }> {
