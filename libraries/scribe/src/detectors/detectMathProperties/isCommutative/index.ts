...
export default function isCommutative(source: string): boolean {
	...
	if (parameterMatch) {
		...
		const param1Count = (source.match(new RegExp(`\\b${param1}\\b`, 'g')) || []).length - 1 // -1 for declaration
		const param2Count = (source.match(new RegExp(`\\b${param2}\\b`, 'g')) || []).length - 1 // -1 for declaration
