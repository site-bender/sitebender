...
export default function extractSignature(signature: FunctionSignature): string {
	const { name, parameters, returnType, generics, isAsync, isGenerator } = signature
	...
	if (isCurried) {
		...
		return formatCurriedSignature(name, parameters, returnType, genericString)
	}
 ...
 */
function formatParameter(param: { name: string; type: string; optional: boolean }): string {
...
function formatCurriedSignature(
	...
	if (parameters.length === 1) {
		return `${name}${genericString}(${formatParameter(parameters[0])}) => ${returnType}`
	}
