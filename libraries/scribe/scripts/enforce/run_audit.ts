...
const manifest: Manifest = JSON.parse(
	await Deno.readTextFile(new URL("./rules.manifest.json", import.meta.url)),
)
...
function matchesPositive(path: string) {
	return manifest.sourceGlobs.some((g) =>
		!g.startsWith("!") && simpleMatch(g, path)
	)
}

function matchesNegative(path: string) {
	return manifest.sourceGlobs.some((g) =>
		g.startsWith("!") && simpleMatch(g.slice(1), path)
	)
}
...
async function analyzeFile(path: string) {
	...
	if (manifest.rules.oneFunctionPerFile && !isTestFile(rel)) {
		const topLevelFunctionCount = lines.filter((l) =>
		...
		if (topLevelFunctionCount !== 1) {
			violations.push({
				...
				detail:
					`Expected exactly 1 exported default function, found ${topLevelFunctionCount}`,
			})
		}
	}
			...
			if (/[^A-Za-z0-9_]\w+\[\]/.test(l) || /:\s*\w+\[\]/.test(l)) {
				if (!l.includes("[] // allow")) {
					violations.push({
						file: rel,
						line: i + 1,
						rule: "arrayGenericNotation",
						detail: "Use Array<T> instead of T[]",
					})
				}
			}
		})
	...
	if (manifest.rules.requireScribeMarker && !isTestFile(rel)) {
		...
		if (exportIdx >= 0) {
			...
			const hasMarker = windowLines.some((l) =>
