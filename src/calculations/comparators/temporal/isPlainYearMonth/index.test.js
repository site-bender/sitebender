...
test("[isPlainYearMonth] (calculations::comparators::scalar) returns value when a plain year-month", () => {
	expect(
		isPlainYearMonth(
...
test("[isPlainYearMonth] (calculations::comparators::scalar) returns an error when not a plain year-month", () => {
	expect(
		isPlainYearMonth(IsPlainYearMonth(Constant("String")("Bob")))(),
	).toMatchObject({
		left: [
			{
				tag: "Error",
