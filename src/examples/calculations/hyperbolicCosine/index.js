import {
	HyperbolicCosine,
	composeOperators,
	FromArgument,
} from "@sitebender/sitebender"

const code = document.querySelector("#code-output")

const HCos = HyperbolicCosine("Number")
const Arg = FromArgument("Number")

// Generate an cosh operation object
const config = HCos(Arg)

// Generate the composed cosh function
const calculate = composeOperators(config)

// Show the config object and the result
const out =
	"The configuration generated by HyperbolicCosine:\n\n" +
	JSON.stringify(config, null, 2)

if (code) {
	code.innerHTML = out
}

// Check the console below
console.log(JSON.stringify(calculate(1), null, 2))
