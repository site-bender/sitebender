...
function EmailField(
	...
	const validator = arr.find((c) =>
		c && typeof c === "object" && "kind" in c
	) as
		| ValidatorConfig
		| undefined
	...
	return {
		...
		children: [
			...
			{
				type: "div",
				props: { class: "help", "aria-live": "polite" },
				children: [help ?? ""],
			},
		],
	}
...
function updateOutputs() {
	...
	if (result.errors.length > 0) {
		errorList.innerHTML = result.errors.map((error) => `<li>${error}</li>`)
			.join("")
