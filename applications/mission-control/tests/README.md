...
```typescript
// tests/behaviors/experimenting/the-workshop/index.ts
import { expect, test } from "playwright-mcp"

test.describe("Code the-workshop behaviors", () => {
	test("can execute code and see results", async ({ page }) => {
		await page.goto("/the-workshop")
	...
	test("the-workshop works offline", async ({ page, context }) => {
		await page.goto("/the-workshop")
...
- MSW or fetch mocks: deny-by-default for the-workshop code; explicit allowlist for docs API examples.

## Offline and Manifest
- Validate presence and correctness of web app manifest.
- Service Worker tests: cache key assets, fallback page works offline, the-workshop executes offline demo.
...
import { expect, test } from "@std/testing/bdd"
