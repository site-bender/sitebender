...
function Required() { return { kind: 'required' } as const }
function EmailFormat() { return { kind: 'emailFormat' } as const }
function MinLength({ length }: { length: number }) { return { kind: 'minLength', length } as const }
function MaxLength({ length }: { length: number }) { return { kind: 'maxLength', length } as const }
...
function Equals({ left, right }: { left: unknown; right: unknown }) { return { kind: 'equals', left, right } }
function Condition({ children }: { children: any }) { return Array.isArray(children) ? children[0] : children }
function IfTrue({ children }: { children: any }) { return { type: 'div', props: { "data-slot": 'iftrue' }, children: Array.isArray(children) ? children : [children] } }
function IfFalse({ children }: { children: any }) { return { type: 'div', props: { "data-slot": 'iffalse' }, children: Array.isArray(children) ? children : [children] } }

function EmailField({ name, label = 'Email', help, required = false, children }:
  { name: string; label?: string; help?: string; required?: boolean; children?: any }) {
  const arr = Array.isArray(children) ? children : (children != null ? [children] : [])
  const validator = arr.find(c => c && typeof c === 'object' && 'kind' in c) as ValidatorConfig | undefined

  return { type: 'div', props: { class: 'field' }, children: [
    { type: 'label', props: { for: name, class: 'label' }, children: [label + (required ? ' *' : '')] },
    { type: 'input', props: { id: name, name, type: 'email', class: 'input', required, 'data-validate': validator }, children: [] },
...
function ConditionalDisplay({ children }: { children: any }) {
  ...
  return { type: 'div', props: { class: 'conditional', 'data-display': condition }, children: rest }
}

const element = { type: 'form', props: { class: 'form' }, children: [
  EmailField({ name: 'email', label: 'Email', help: 'We’ll never share your email.', required: true, children: [
    Validation({ children: And({ children: [Required(), EmailFormat(), MinLength({ length: 6 }), MaxLength({ length: 100 })] }) as ValidatorConfig })
  ] }),
  ConditionalDisplay({ children: [
    Condition({ children: Equals({ left: 1, right: 1 }) }),
    IfTrue({ children: { type: 'div', props: {}, children: ['This will be displayed if the condition is true.'] } }),
    IfFalse({ children: { type: 'div', props: {}, children: ['This will be displayed if the condition is false.'] } })
  ] })
] }
