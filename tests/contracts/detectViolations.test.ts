...
import { detectViolations } from "../../libraries/contracts/enforcement/index.ts"
...
Deno.test("Detect Violations: Parser should not import from Envoy", async () => {
	try {
		const { stdout } = await execAsync(
			`grep -r "from ['\\"].*envoy" libraries/parser/src/ 2>/dev/null || true`
		)
		
		if (stdout.trim()) {
			throw new Error(`Parser is importing from Envoy:\n${stdout}`)
		}
	} catch (error) {
		// Expected to not find anything
		if (error && error.stdout && error.stdout.trim()) {
...
Deno.test("Detect Violations: Toolkit should not import any @sitebender library", async () => {
	try {
		const { stdout } = await execAsync(
			`grep -r "from ['\\"]@sitebender" libraries/toolkit/src/ 2>/dev/null || true`
		)
		
		if (stdout.trim()) {
			throw new Error(`Toolkit is importing @sitebender libraries:\n${stdout}`)
		}
	} catch (error) {
		// Expected to not find anything
		if (error && error.stdout && error.stdout.trim()) {
...
Deno.test("Detect Violations: Foundry should not import any @sitebender library", async () => {
	try {
		const { stdout } = await execAsync(
			`grep -r "from ['\\"]@sitebender" libraries/foundry/src/ 2>/dev/null || true`
		)
		
		if (stdout.trim()) {
			throw new Error(`Foundry is importing @sitebender libraries:\n${stdout}`)
		}
	} catch (error) {
		// Expected to not find anything
		if (error && error.stdout && error.stdout.trim()) {
...
Deno.test("Detect Violations: No library should use regex to parse TypeScript", async () => {
	const libraries = ["envoy", "prover", "components", "engine"]
